...
```{r}
# Create data frame for pre-exposure IO. 
...
```{r plot-fit-PSE, fig.width=13, fig.height=7, out.width="100%", fig.cap="(ref:plot-fit-PSE)"}
p.fit_1to7 +
  p.fit_7to9 +
  (p.PSE_1to9 +
     geom_label(
     ...
     annotate(
       ...
       label = d.psychometric_intercept.slope.PSE.median %>%
         mutate(
           true_shift = round(predictedPSE_unscaled - PSE_unscaled),
           true_shift = paste0(ifelse(true_shift > 0, "+", ""), true_shift, " (100%)")) %>%
...
make_hyp_table(
  ...
  pack_rows("Comparing average PSE against idealized pre-exposure listener (Test block 1)", 4, 4)
```
...
Critically, participants' categorization functions during Test 1 closely matched the predictions of the idealized pre-exposure listener, shown as a gray band in Figure \@ref(fig:plot-fit-PSE)A. This is confirmed by a Bayesian hypothesis test summarized in the final section of Table \@ref(tab:hypothesis-table-prediction1), which compared the average PSE of all conditions, against the mean PSE of the five cross-validated idealized pre-exposure listeners (BFs $\ge$ `r get_bf(fit_test_nested_within_condition_and_block, str_c("(-mu2_IpasteCondition.ExposureBlocksepEQxShift0x1/mu2_IpasteCondition.ExposureBlocksepEQxShift0x1:VOT_gs + 
          -mu2_IpasteCondition.ExposureBlocksepEQxShift10x1/mu2_IpasteCondition.ExposureBlocksepEQxShift10x1:VOT_gs +
          -mu2_IpasteCondition.ExposureBlocksepEQxShift40x1/mu2_IpasteCondition.ExposureBlocksepEQxShift40x1:VOT_gs)/3 -", predictedPSE_scaled_prior, "= 0"), bf = T)`). This demonstrated that a simple ideal observer trained on the phonetic distributions that a 'typical' listener might have previously experienced provides a decent model of listeners' perception prior to additional exposure [see also, e.g., @kronrod2016; @nearey-assmann2007; @tan2021; @xie2021cognition]. Notably, this is achieved with 0 degrees of freedom that are fit to listeners' responses: the idealized pre-exposure listeners were trained solely on a phonetic database [@chodroff-wilson2018], and not fit to listeners' responses.
...
```{r get-talker-specific-PSEs-for-plot}
...
m_IA.VOT_f0_vowelduration_informedprior <-
  fit_ideal_adaptor(
    ...
    file_refit = if (RESET_MODELS) "always" else "on_change")
```

```{r get-and-summarise-PSE-predictions-of-IA}
# Get logistic model predictions of the IA's categorization then prepare the data for plotting
# We take this somewhat indirect way to calculate the predicted PSE of the IA in order to make
# sure that the PSE of the IA is estimated with the same procedure that we apply to listeners 
# (assuming linearity over VOT).

# First get the IA predictions in log-odds
if (file.exists("../models/d.IA_predicted_test_informedprior.rds") & !RESET_MODELS) {
  d.IA_predicted_logodds <- readRDS("../models/d.IA_predicted_logodds.rds")
} else {
  d.IA_predicted_logodds <-
    get_IBBU_predicted_response(
      model = m_IA.VOT_f0_vowelduration_informedprior,
      data = 
        prep_data_for_IBBU_prediction(m_IA.VOT_f0_vowelduration_informedprior, prep_test = T) %>%
      ...
      groups = get_group_levels(m_IA.VOT_f0_vowelduration_informedprior),
      logit = T,
      seed = GLOBAL_SEED) 
 
  saveRDS(d.IA_predicted_logodds, "../models/d.IA_predicted_logodds.rds")
}

if (file.exists("../models/d.IA_predicted_PSE_informedprior.rds") & !RESET_MODELS) {
  d.IA_predicted_PSE <- readRDS("../models/d.IA_predicted_PSE_informedprior.rds")
} else {
  d.IA_predicted_PSE <-
    fit_logistic_regression_to_model_categorization(
      d.IA_predicted_logodds %>% mutate(Predicted_posterior = plogis(Predicted_posterior))) %>%
    group_by(group) %>%
    median_hdci(PSE) %>%
    mutate(rows = ifelse(group == "no exposure", 3, 1)) %>%
    uncount(rows) %>%
    mutate(
      Condition.Exposure = ifelse(group != "no exposure", gsub("^.*(Shift[0-9]+)_.*$", "\\1", group), c("Shift0", "Shift10", "Shift40")),
      Block = ifelse(group != "no exposure", str_replace(group, ".*(\\d)", "\\1"), group),
      Block = factor(ifelse(group == "no exposure", 1, Block))) %>%
    add_block_labels()

  saveRDS(d.IA_predicted_PSE, "../models/d.IA_predicted_PSE_informedprior.rds")
}
```
...
IA.prediction <-
  d.IA_predicted_logodds %>% 
  mutate(Predicted_posterior = plogis(Predicted_posterior)) %>%
  group_by(group, VOT) %>%
...
# Prep data to obtain IA mode of predicted logodds by condition and VOT up to test block 4
...
fit_test.IA_predicted_logodds <-
  brm(
    bf(
      Response.Voiceless ~ 0 + IA_predicted_logodds_gs +
        (1 | ParticipantID) + (1 | Item.MinimalPair)),
    data = d.IA_predicted_logodds.mode,
...
summarize.NIW_ideal_adaptor_stanfit <- function(
  ...
  get_draws(x, groups = groups, summarize = F) %>%
...
fit_test.IA_predicted_logodds.prior.change.interaction_with_RLs <-
  update(
    ...
    file ="../models/test_IA_predicted_logodds.prior.change.interaction_with_RLs.rds",
    recompile = TRUE)
