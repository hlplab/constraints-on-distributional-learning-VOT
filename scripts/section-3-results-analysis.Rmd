...
Figure \@ref(fig:plot-IA-human-PSE) shows the PSEs inferred from the ideal adaptor, along with listeners' PSEs (the same as in Figure Figure \@ref(fig:plot-fit-PSE)C).  

(ref:plot-IA-human-PSE) Comparison of shifts in listener PSEs over the first four test blocks against that expected by an ideal adaptor (an idealized learner who adapts to talkers by integrating cue statistics of the exposure talker with prior beliefs about category cue means and variances). Ribbons indicate the 95% highest posterior density interval

```{r plot-IA-human-PSE, fig.width=5, fig.height=3, fig.cap="(ref:plot-IA-human-PSE)"}
d.estimates %>%
  filter(Block %in% c(1:7)) %>% 
  ggplot(
    aes(
      x = Block.plot_label, y = PSE_median,
      ymin = PSE_lower, ymax = PSE_upper,
      colour = Condition.Exposure, group = Condition.Exposure)) +
  geom_rect(
    data = tibble(
