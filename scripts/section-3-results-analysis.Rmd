...
my_priors <- c(
  ...
  set_prior("student_t(3, 0, 10)", dpar = "mu2", coef = "VOT_gs"),
...
d.true_shift <- 
  ...
  mutate(
    ...
    proportion_shift = abs(PSE_median - PSE_block1)/true_shift)
...
my_priors <- c(
  prior(student_t(3, 0, 2.5), class = "b", dpar = "mu2"),  
  set_prior("student_t(3, 0, 10)", dpar = "mu2", coef = "Block1:VOT_gs"),
  set_prior("student_t(3, 0, 10)", dpar = "mu2", coef = "Block3:VOT_gs"),
  set_prior("student_t(3, 0, 10)", dpar = "mu2", coef = "Block5:VOT_gs"),
  set_prior("student_t(3, 0, 10)", dpar = "mu2", coef = "Block7:VOT_gs"),
  set_prior("student_t(3, 0, 10)", dpar = "mu2", coef = "Block8:VOT_gs"),
  set_prior("student_t(3, 0, 10)", dpar = "mu2", coef = "Block9:VOT_gs"),
  prior(cauchy(0, 2.5), class = "sd", dpar = "mu2"),
  prior(lkj(1), class = "cor")
)
...
fit_mix_test_nested_block <- 
  ...
  bf(
    ...
    file ="../models/test-nested_block-sample-priorSD10-0.995.rds")
...
d_for_analysis %<>%
  ungroup() %>% 
  mutate(
    VOT.CCuRE = get_CCuRE_VOT(
    data = d.chodroff_wilson, 
    newdata = d_for_analysis %>% 
      ungroup() %>% 
      mutate(VOT = Item.VOT, f0_Mel = Item.F0_Mel, Talker = "new")),
    F0_Mel.CCuRE = get_CCuRE_f0_Mel(
      data = d.chodroff_wilson, 
    newdata = d_for_analysis %>% 
      ungroup() %>% 
      mutate(VOT = Item.VOT, f0_Mel = Item.F0_Mel, Talker = "new")),
...
shift10_cov_d <- 
  ...
  mutate(covariance = map_dbl(data, ~ cov(.x$VOT.CCuRE, .x$F0_Mel.CCuRE))) %>%
  pull(covariance))[1]
...
shift10_ellipse <- 
  ...
  group_by(category) %>% 
  summarise(across(c(VOT.CCuRE, F0_Mel.CCuRE), list(mean = mean, var = var), .names = "{.col}.{.fn}")) %>% 
  mutate(
    cov = ifelse(category == "/d/", shift10_cov_d, shift10_cov_t), 
    mu = map2(VOT.CCuRE.mean, F0_Mel.CCuRE.mean, ~ c("VOT" = .x, "f0_Mel" = .y)), 
    Sigma = pmap(list(
      VOT.CCuRE.var, F0_Mel.CCuRE.var, cov), ~ matrix(c(..1, ..3, ..3, ..2), 2, 2, dimnames = list(cues, cues)))) %>%
  dplyr::select(c(category, mu, Sigma)) %>% 
  crossing(level = .95) %>%
  group_by(category) %>%
