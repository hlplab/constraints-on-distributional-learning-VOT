...
## Results
Figure \@ref(fig:plot-fit-slope-PSE) summarizes the results. Panels A and B show participants’ categorisation responses during exposure and test blocks, along with the categorisation function estimated from those responses via a mixed-effects psychometric model that we describe next. These panels facilitate comparison between exposure conditions within each block. Panels C and D show the slope and point of subject equality (PSE)---i.e., the point at which participants are equally likely to respond "d" and "t"---of the categorisation function across blocks and conditions. These panels facilitate comparison across blocks within each exposure condition.
...
# make IOs for each exposurecondition  
ios <- 
  tibble(
...
```{r fit-nested-blocks-simple-effects, results='hide', warning=FALSE}
...
chodroff.mean_VOT <- mean(d.chodroff_wilson.selected$VOT)
chodroff.sd_VOT <- sd(d.chodroff_wilson.selected$VOT)
chodroff.mean_vowel_duration <- mean(d.chodroff_wilson.selected$vowel_duration)
chodroff.sd_vowel_duration <- sd(d.chodroff_wilson.selected$vowel_duration)

# get estimates from speech rate model to predict VOT
intercept.speech_rate <- 
  get_speech_rate_model(d.chodroff_wilson.selected)$estimate[1] 
slope.speech_rate <- 
  get_speech_rate_model(d.chodroff_wilson.selected)$estimate[2]
