...
io <- 
  make_VOT_IOs_from_exposure(
    d_for_analysis %>% 
      ...
      rename(VOT = Item.VOT, f0 = Item.F0_5ms_Mel))
...
cond_means_test_exposure <- d_for_analysis %>%
  ...
  summarise(across(c(Item.VOT, Item.F0_5ms_Mel), mean, .names = "{.col}_mean_test_exposure"))

cond_means_exposure <- d_for_analysis %>%
  filter(Phase == "exposure") %>%
  group_by(Condition.Exposure) %>%
  summarise(across(c(Item.VOT, Item.F0_5ms_Mel), mean, .names = "{.col}_mean_exposure"))
...
d_for_analysis %<>% 
  ...
  mutate(
    ...
    Item.F0_5ms_Mel_centered = case_when(
      Condition.Exposure == "Shift0" ~ Item.F0_5ms_Mel + (chodroff.mean_f0_Mel - cond_means$Item.F0_5ms_Mel_mean_exposure[1]),
      Condition.Exposure == "Shift10" ~ Item.F0_5ms_Mel + (chodroff.mean_f0_Mel - cond_means$Item.F0_5ms_Mel_mean_exposure[2]),
      Condition.Exposure == "Shift40" ~ Item.F0_5ms_Mel + (chodroff.mean_f0_Mel - cond_means$Item.F0_5ms_Mel_mean_exposure[3]))) 
...
p.density <- 
  ...
  geom_point(
    data = d_for_analysis %>%
      ...
      summarise(across(c(Item.VOT_centered, Item.F0_5ms_Mel_centered), mean)),
    mapping = aes(x = Item.VOT_centered, y = Item.F0_5ms_Mel_centered, colour = Condition.Exposure, shape = category),
    size = 2,
    alpha = 0.8,
  inherit.aes = F,
  show.legend = T) +
...
shift10_cov_d <- 
  ...
  mutate(covariance = map_dbl(data, ~ cov(.x$Item.VOT_centered, .x$Item.F0_5ms_Mel_centered))) %>%
  pull(covariance))[1]
...
shift10_ellipse <- 
  ...
  group_by(category) %>% 
  summarise(across(c(Item.VOT_centered, Item.F0_5ms_Mel_centered), list(mean = mean, var = var), .names = "{.col}.{.fn}")) %>% 
  mutate(
    cov = ifelse(category == "/d/", shift10_cov_d, shift10_cov_t), 
    mu = map2(Item.VOT_centered.mean, Item.F0_5ms_Mel_centered.mean, ~ c("VOT" = .x, "f0_Mel" = .y)), 
    Sigma = pmap(list(
      Item.VOT_centered.var, Item.F0_5ms_Mel_centered.var, cov), ~ matrix(c(..1, ..3, ..3, ..2), 2, 2, dimnames = list(cues, cues)))) %>%
