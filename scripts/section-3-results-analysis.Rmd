...
Critically, participants' categorization functions during Test 1 closely matched the predictions of the idealized pre-exposure listener, shown as a gray band in Figure \@ref(fig:plot-fit-PSE)A. This is confirmed by Bayesian hypothesis tests summarized in the bottom half of Table \@ref(tab:hypothesis-table-prediction1), which compared participants' PSE against the mean PSE of the five cross-validated idealized pre-exposure listeners (BF $\ge$ `r get_bf(fit_test_nested_within_condition_and_block, str_c("(-mu2_IpasteCondition.ExposureBlocksepEQxShift0x1/mu2_IpasteCondition.ExposureBlocksepEQxShift0x1:VOT_gs) -", predictedPSE_scaled_prior, "= 0"), bf = T)`)^[All point hypotheses tests were tested by taking the ratio of posterior densities to prior densities at the null with kernel density estimation]. This demonstrated that a simple ideal observer trained on the phonetic distributions that a 'typical' listener might have previously experienced provides a decent model of listeners' perception prior to additional exposure [see also, e.g., @kronrod2016; @nearey-assmann2007; @tan2021; @xie2021cognition]. Notably, this is achieved with 0 degrees of freedom that are fit to listeners' responses: the ideal observers were trained solely on a phonetic database.
