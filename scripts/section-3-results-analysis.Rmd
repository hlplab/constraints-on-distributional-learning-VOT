...
### Regression analyses
Figures \@ref(fig:plot-fit-slope-PSE)A-B summarize participants’ categorisation responses during exposure and test blocks, depending on the exposure condition and VOT. We analyzed participants' categorisation responses during exposure and test blocks in two separate Bayesian mixed-effects psychometric models. These models account for attentional lapses while estimating participants' categorisation functions. Though we confirmed for the present case that all results would replicate in a simple mixed-effects logistic regression [@jaeger2008categorical], ignoring attentional lapses---while commonplace in research on speech perception [but see @clayards2008perception; @kleinschmidt2016you]---can lead to biased estimates of categorization boundaries [@prins2012psychometric; @wichmann2001psychometric]. The models were fit using {\tt brms}} [@R-brms_a] in R [@R; @RStudio, for details, see SI, \@ref(sec:analysis-approach)]. 

Here we focus on the model fit to the test blocks, which were identical within and across exposure conditions. Analyses of the exposure blocks are reported in the SI, and replicate all effects found in the test blocks. Tables \@ref(tab:XXX)-\@ref(tab:XXX) present Bayesian hypothesis tests that assess our hypotheses. As shown in Table \@ref(tab:XXX) 
