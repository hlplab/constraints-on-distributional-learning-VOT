...
d.chodroff_wilson.isolated <- map(.x = c(1:5), ~ 
...
```
### Differences in block-to block *slope* changes by condition




```{r hypothesis-table-interaction-condition-block, "TEMPORARY PLACEMENT OF INTERACTION TABLE HERE", results='asis'}
...
```{r "TEMPORARY PLACEMENT OF INTERACTION TABLE HERE1", results='asis'}
...
```{r "TEMPORARY PLACEMENT OF INTERACTION TABLE HERE2", results='asis'}
...
To further illustrates this point, the gray band in Figure \@ref(fig:plot-fit-slope-PSE)D shows the range of PSEs predicted by Bayesian ideal observers trained on the distribution of VOT, f0, and vowel duration for isolated word productions in Figure \@ref(fig:exposure-means-database-matrix-plot). Following @xie2023, the ideal observers included perceptual noise [estimates taken from @kronrod2016, for details, see SI, \@ref(sec:prior-idealized-listeners)]. We emphasize that the seemingly close fit between ideal observers' predictions and listeners' behavior should be interpreted with caution: the ideal observer predictions shown in Figure \@ref(fig:plot-fit-slope-PSE)D are based on a single phonetic database. We chose this database because it is large compared to other phonetically annotated databases, and because it contains recordings that were obtained under similar conditions as our stimuli---conditions likely to, for example, elicit some degree of hyper-articulation. It should, however, be kept in mind that the database reflects the speech of talkers recruited at a single university campus in the US, and stimuli selected to cover specific phonetic contexts rather than to be representative of a typical listener's everyday speech input. <!-- TO DO: are the points in this sentence correct? --> 

<!-- POSSIBLE ADDITION BUT IT WOULD PROBABLY BE MISREAD BY SOME READERS, AND PROVIDE A FALSE SENSE OF THE STRENTH OF THIS RESULT: Credible intervals of the pre-exposure PSEs in Test 1 overlapped with the predictions of the ideal observer model, and support for the hypothesis that listeners' pre-exposure PSEs were identical to these predictions was strong (BF = XXX). 

[point hypothesis for intercept in block 1 == prediction using model that nests as follows: 0 + block / (1 + condition * VOT) ...

BUT I THINK PERHAPS MORE USEFUL IS TO PLOT AN INTERVAL FOR IO PREDICTIONS, RATHER THAN A SINGLE LINE? (AND THEN NOT USE BFs)
--> 

(ref:exposure-means-database-matrix-plot) Placement of exposure stimuli relative to an estimate of typical phonetic distributions for `r nrow(d.chodroff_wilson)` word-initial /d/ and /t/ productions by `r length(unique(d.chodroff_wilson$Talker))` female L1 talkers of US English in @chodroff-wilson2018. After voice onset time, f0 and vowel duration are two of the most informative cues to word-initial /d/-/t/ in L1 US English. For details, see SI \@ref(sec:SI-phonetic-data). Colored labels show the category means of the exposure conditions.
...
d.exposure_surprisal <-
  ...
  cross_join(
    io %>%
      filter(io %in% c("Shift0", "Shift10", "Shift40", "prior")) %>% 
      select(-x) %>%
