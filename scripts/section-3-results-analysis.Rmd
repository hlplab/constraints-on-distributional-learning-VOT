...
hypotheses_contrasts_within_condition <- c(
)
...
table2 %>% 
  mutate(across(where(is.numeric), ~ round(., digits = 3))) %>% 
  rename_all(~gsub("\\.", " ", .)) %>% 
  kbl(caption = "Comparing conditions in test blocks 1 to 4", align = align_tab(table2)) %>% 
  kable_styling(full_width=FALSE, 
                bootstrap_options = c("striped", "hover", "condensed")) %>% 
  column_spec(1, width = "8em")
```
...
```{r fit-nested-models-for-intercepts-slopes-simple-effects, warning=FALSE, message=FALSE}
...
```{r prepare-plot-intercepts-slopes-1to7}
