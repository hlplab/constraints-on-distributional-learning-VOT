...
# transpose the VOT test mean into exposure units for correction of exposure intercepts
VOT.mean_in_exposure_scale <- (VOT.mean_test - VOT.mean_exposure)/(2 * VOT.sd_exposure)

d.estimates <-
  full_join(
    fit_mix_test_nested %>% get_intercepts_and_slopes(),
    fit_mix_exposure_nested %>% get_intercepts_and_slopes() %>% 
      # make corrections to intercepts in exposure blocks to account for prior effects
       mutate(Intercept_corrected = Intercept + fixef(fit_mix_exposure)[[3]] * VOT.mean_in_exposure_scale,
         Intercept = Intercept_corrected) %>% 
  select(-c(Intercept_corrected))) %>%
  mutate(
    PSE = ifelse(
      Block %in% c(2, 4, 6),
  ...
  summarise(
    across(
      ...
      list(lower = ~ quantile(.x, probs = .025), median = ~ round(quantile(.x, probs = .5), 3), upper = ~ quantile(.x, probs = .975)))) 
```
...
d.true_shift <- 
  ...
  mutate(
    mean_PSE_block1 = mean(c(d.estimates[10][[1]][1], d.estimates[10][[1]][10], d.estimates[10][[1]][19])),
    true_shift = PSE.io_predicted - mean_PSE_block1,
    proportion_shift = abs((PSE_median - mean_PSE_block1)/true_shift),
    proportion_shift = ifelse(PSE_median >= mean_PSE_block1 & Condition.Exposure != "Shift40", 0, proportion_shift))
...
p.PSE_1to7 <-     
  ...
  show.legend = F) +
  geom_hline(
    data = d.io.categorization %>% filter(Condition.Exposure != "prior"),
