...
p.slope_1to7 <-
  ...
  geom_hline(
    data = d.io.categorization %>% 
      filter(Condition.Exposure %in% c("prior", "Shift10")),
    # Using UNscaled slope because slopes are comparable across scales anyway and this is more intuitive
...
```{r, fig.caption="TEMPORARILY PUTTING SLOPES TABLE HERE", results='asis'}
...
d.chodroff_wilson %>%
  ...
  facet_matrix(vars(c(VOT, f0_Mel, vowel_duration)), layer.lower = c(3, 4:5), layer.diag = 2,
               ...
               .cols = c(VOT = "VOT", f0_Mel = "F0 (Mel)", vowel_duration = "Vowel duration"))) +
  theme(legend.position = "top")
```
