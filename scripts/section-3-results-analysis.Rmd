...
The model is described in detail in the SI (\@ref(sec:ideal-adaptor)), including graphical models [for an introduction, see @xie2023]. In order to model the effects of all three cues considered in the idealized models (VOT, f0, and vowel duration), we extended the model to operate over multivariate categories, and fit it to listeners' responses in our experiment. Unlike in previous work, we fixed the models prior beliefs based on the same phonetic database used for the idealized pre-exposure listener [cf. @kleinschmidt-jaeger2016]. Specifically, we used prior beliefs that had the same expected distributions of VOT, f0, and vowel duration for /d/ and /t/ as in the idealized listener model. This substantially reduces the number of free parameters in the model that are fit to participants' responses: in addition to a lapse rate, the ideal adaptor we fit only had two degrees of freedom. These two parameters describe the strength of participants' prior beliefs about the category means and (co)variances, respectively. These parameters describe both the uncertainty that listeners have about the unfamiliar talker's phonetic distributions prior to exposure *and* the the rate at which the model updates its beliefs about the exposure distributions with each new observation from that talker. This captures how prior expectations both guide and constrain distributional learning [for discussion, see @kleinschmidt-jaeger2015; @kleinschmidt2020]. We used the \texttt{MVBeliefUpdatr} package [@R-MVBeliefUpdatr]. For further detail, we refer to the SI (\@ref(sec:ideal-adaptor)).
...
#     family = bernoulli("logit"),
#     control = list(adapt_delta = .999),
#     file ="../models/test_IA_predicted_logodds_with_standardREs.rds",
#     file_refit = "on_change")
