...
make_hyp_table(
  ...
  c(
    ...
    "$|\\Delta(PSE_{idealized_{+20ms}}, PSE_{actual_{+20ms}})| > 0$"),
  caption = "Did participants converge against the PSE expected from idealized learner? This table compares changes in participants' categorization function against those expected from idealized learners. The first set of tests assess whether participants converged against tFor identical hypothesis tests for all test blocks, see SI (\\ref{sec:convergence-test-all}).",
  col1_width = "20em") %>%
...
The model is described in detail in the SI (\@ref(sec:ideal-adaptor)), including graphical models [for an introduction, see @xie2023]. In order to model the effects of all three cues considered in the idealized models (VOT, f0, and vowel duration), we extended the model to operate over multivariate categories, and fit it to listeners' responses in our experiment. We used the \texttt{MVBeliefUpdatr} package [@R-MVBeliefUpdatr] to define and fit the model. For further detail, we refer to the SI (\@ref(sec:ideal-adaptor)).

Unlike in previous work, we fixed the models prior beliefs based on the same phonetic database used for the idealized pre-exposure listener [cf. @kleinschmidt-jaeger2016]. Specifically, we used prior beliefs that had the same expected distributions of VOT, f0, and vowel duration for /d/ and /t/ as in the idealized listener model. This substantially reduces the number of parameters in the model that are fit to participants' responses: in addition to a lapse rate ($\lambda$), the ideal adaptor we fit only had two degrees of freedom. These two parameters ($\kappa_0$ and $\nu_0$) describe the strength of participants' prior beliefs about the category means and (co)variances, respectively. These two parameters describe both the uncertainty that listeners have about the unfamiliar talker's phonetic distributions prior to exposure (or, equivalently under this model, how relevant listeners believe their prior expectations to be for the unfamiliar talker). The same two parameters also determine the the rate at which the model updates its beliefs about the exposure distributions with each new observation from that talker. This captures how prior expectations both guide and constrain distributional learning [for discussion, see @kleinschmidt-jaeger2015; @kleinschmidt2020]. Specifically, $\kappa_0$ and $\nu_0$ can be understood as pseudocounts that describe the weighting of prior expectations relative to the observed data. For example, after $\kappa_0$ observed /d/s from the unfamiliar talker, listeners' beliefs about the VOT, f0, and vowel duration values of the category mean of /d/ will be a 50/50 mixture of their prior beliefs and the means observed in the $\kappa_0$ /d/s that have been observed from the unfamiliar talker.
...
if (file.exists("../models/d.IA_predicted_logodds.csv")) {
  ...
  d.IA_predicted_logodds <-
    ...
    rename(Item.VOT = VOT, IA_predicted_logodds = Predicted_posterior) %>%
    right_join(
      d.for_analysis %>%
        mutate(Block = factor(Block)),
    ...
    group_by(Item.VOT) %>%
    mutate(
      IA_predicted_logodds.prior = first(IA_predicted_logodds),
