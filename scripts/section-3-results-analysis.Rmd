    ...
    VOT.CCuRE = remove_speechrate_effect_from_cue(
      ...
      newdata = d.for_analysis %>% 
        ...
        mutate(VOT = Item.VOT, Talker = "new",
               vowel_duration = Item.Vowel_duration),
      cue = "VOT"),
    f0_Mel.CCuRE = remove_speechrate_effect_from_cue(
      data = d.chodroff_isolated, 
      newdata = d.for_analysis %>% 
        mutate(vowel_duration = Item.Vowel_duration) %>% 
        ungroup() %>% 
...
d_breaks <- 
  density_quantiles(
    x = d.chodroff_isolated %>% filter(category == "/d/") %>% pull(VOT),
    y = d.chodroff_isolated %>% filter(category == "/d/") %>% pull(f0_Mel),
    quantiles = quantile_levels)
t_breaks <- 
  density_quantiles(
    x = d.chodroff_isolated %>% filter(category == "/t/") %>% pull(VOT),
    y = d.chodroff_isolated %>% filter(category == "/t/") %>% pull(f0_Mel),
    quantiles = quantile_levels)
