...
io.accuracy <- 
  ...
  mutate(Condition.Exposure = factor(rep(c("Shift0", "Shift10", "Shift40"), 6))) %>% 
  pivot_longer(
    cols = starts_with("categorization"),
    names_to = "decision",
    values_to = "accuracy") %>% 
  mutate(
    Condition.io = ifelse(str_detect(Condition.io, "prior"), str_replace(Condition.io, "(prior\\d).*", "\\1"), Condition.io),
