...
```{r hypothesis-table1, results='asis'}
...
table1 <- 
  make_hyp_table(
    hyp_interactions_readable,
    hyp_interactions,
    caption = "Was there incremental change from test block 1 to 4? This table summarizes the interactions between exposure condition and block, whether the differences between exposure conditions changed from test block to test block.") %>% 
...
```{r hypothesis-table2, results='asis'}
...
```{r hypothesis-table3, results='asis'}
# hypotheses test of proportionality (Is the diff between +40 and baseline < 4 * difference between +10 and baseline)
hyp_proportional <- c(
  "mu2_Block3:Condition.Exposure_Shift40vs.Shift10 + mu2_Block3:Condition.Exposure_Shift10vs.Shift0 < 4 * mu2_Block3:Condition.Exposure_Shift10vs.Shift0",
  "mu2_Block5:Condition.Exposure_Shift40vs.Shift10 + mu2_Block5:Condition.Exposure_Shift10vs.Shift0 < 4 * mu2_Block5:Condition.Exposure_Shift10vs.Shift0",
  "mu2_Block7:Condition.Exposure_Shift40vs.Shift10 + mu2_Block7:Condition.Exposure_Shift10vs.Shift0 < 4 * mu2_Block7:Condition.Exposure_Shift10vs.Shift0"
)
...
```{r hypothesis-table4, results='asis'}
...
hyp_test456_readable <- tibble(Hypothesis_readable = c(
  ...
  "Predicted change from test block 4 to 5",
  "Predicted change from test block 5 to 6",
  "Predicted total change from test block 4 to 6",  
...
table4 <- make_hyp_table(hyp_test456_readable, hyp_test456, caption = "Effects of repeated testing (test blocks 4 to 6)" ) %>%
...
quantile_levels <- c(0, .05, .25, .5, .75, .95)
...
p.density <-
  ...
  #scale_linetype_manual(labels = c("/d/", "/t/"), values = c(2, 1)) +
  theme(legend.position = "top") +
  #plot centered means
  geom_point(
    data = d_for_analysis %>%
  ...
  #     group_by(category) %>% 
  #     summarise(across(c(VOT_centered, f0_Mel_centered), mean, .names = "{.col}.mean")),
  #   aes(x = VOT_centered.mean, y = f0_Mel_centered.mean, shape = category),
  #   inherit.aes = F,
  #   show.legend = F) +
