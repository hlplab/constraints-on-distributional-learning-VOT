...
caption <- paste("B :", "the final three test blocks without intervening exposure. More description", collapse = " ")

my_text <- ggplot(data = tibble(x = 0, y = 1, label = caption)) +
...
table1 <- cbind(hyp_interactions_readable, hyp_interactions) %>%
  ...
  rename_all(~ gsub("\\.", " ", .)) %>%
  kbl(caption = "Was there incremental change from test block 1 to 4? This table summarizes the interactions between exposure condition and block, whether the differences between exposure conditions changed from block to block.", align = align_tab(table1)) %>%
...
table2 <- cbind(hyp_contrasts_nested_block_readable, hyp_contrasts_nested_block) %>%
  ...
  mutate(across(where(is.numeric), ~ round(., digits = 3))) %>%
  rename_all(~gsub("\\.", " ", .)) %>%
  kbl(caption = "When did exposure begin to affect participants' categorization responses? This table summarizes the simple effects of the exposure conditions for each of the first four test blocks.", align = align_tab(table2)) %>%
  kable_styling(full_width=FALSE,
                bootstrap_options = c("striped", "hover", "condensed")) %>%
  column_spec(1, width = "8em") %>%
...
```{r make-hypothesis-table3}
# hypotheses test of proportionality (Is the diff between +40 and baseline < 4 * difference between +10 and baseline)
hyp_proportional <- c(
  "mu2_Block1:Condition.Exposure_Shift40vs.Shift10 + mu2_Block1:Condition.Exposure_Shift10vs.Shift0 < 4 * mu2_Block1:Condition.Exposure_Shift10vs.Shift0",
  "mu2_Block3:Condition.Exposure_Shift40vs.Shift10 + mu2_Block3:Condition.Exposure_Shift10vs.Shift0 < 4 * mu2_Block1:Condition.Exposure_Shift10vs.Shift0",
  "mu2_Block5:Condition.Exposure_Shift40vs.Shift10 + mu2_Block5:Condition.Exposure_Shift10vs.Shift0 < 4 * mu2_Block1:Condition.Exposure_Shift10vs.Shift0",
  "mu2_Block7:Condition.Exposure_Shift40vs.Shift10 + mu2_Block7:Condition.Exposure_Shift10vs.Shift0 < 4 * mu2_Block1:Condition.Exposure_Shift10vs.Shift0"
)

# translate hypotheses into intelligible statements
hyp_proportional_readable <- tibble(Hypothesis = c(
...
hyp_proportional <- hypothesis(fit_mix_test_nested_block, hyp_proportional)
hyp_proportional <- hyp_proportional$hypothesis %>%
  select(-Star)

table3 <- cbind(hyp_proportional_readable, hyp_proportional) %>%
  select(-2) %>%
  mutate(across(where(is.numeric), ~ round(., digits = 3))) %>%
  rename_all(~gsub("\\.", " ", .)) %>%
  kbl(caption = "Is the shift in +40 from baseline proportional to the magnitude of shift in the exposure distribution?", align = align_tab(table2)) %>%
  kable_styling(full_width=FALSE,
                bootstrap_options = c("striped", "hover", "condensed")) %>%
  column_spec(1, width = "8em") %>%
  pack_rows("Test block 1", 1, 1) %>%
  pack_rows("Test block 2", 2, 2) %>% 
  pack_rows("Test block 3", 3, 3) %>% 
  pack_rows("Test block 4", 4, 4)
table3
```
