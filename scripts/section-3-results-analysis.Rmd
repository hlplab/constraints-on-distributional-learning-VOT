...
make_VOT_IOs_from_exposure <- function(data){ 
  ...
  # first distinct the data to one full lists of each condition.
...
d.true_shift <- 
  ...
  left_join(d.io.categorisation %>% 
  filter(Condition.Exposure != "prior") %>% 
  select(PSE) %>% 
    rename(PSE.io_predicted = PSE)) %>%
  mutate(
    PSE_block1 = case_when(
      Condition.Exposure == "Shift0" ~ d.estimates[10][[1]][1],
      Condition.Exposure == "Shift10" ~ d.estimates[10][[1]][10],
      Condition.Exposure == "Shift40" ~ d.estimates[10][[1]][19]),
    true_shift = PSE.io_predicted - PSE_block1,
    proportion_shift = abs(PSE_mean - PSE_block1)/true_shift)
...
p.PSE_1to7 <-     
  ...
  geom_hline(
    ...
    linetype = 2, linewidth = .8, alpha = 0.5) +
  geom_text(
   data = d.true_shift %>% group_by(Condition.Exposure, true_shift) %>% summarise(),
   mapping = aes(
    x = 10.25,
    y = c(24, 35, 65),
   ...
   fontface = "bold",
   size = 3,
   position = position_dodge2v(1), 
   inherit.aes = F) +
  annotate(
    geom = "text",
    x = 10.05,
    y = 33.5,
    label = round(d.io.categorisation$PSE[1], 2),
    size = 3,
    colour = "grey",
    fontface = "bold") +
    coord_cartesian(
...
fit_mix_test_nested_condition <- 
  brm(
    ...
    sample_prior = "yes",
    control = list(adapt_delta = .995),
    file ="../models/Exp-AE-DLVOT-nested-condition-sample-prior.rds")
...
simple_effects_block <-
  tidy(fit_mix_test_nested_condition, effects = "fixed") %>%
  filter(term != "theta1_(Intercept)")
```
...
hyp_contrasts_nested_block <-
  c(
    ...
    "mu2_Block1:Condition.Exposure_Shift40vs.Shift10 = mu2_Block1:Condition.Exposure_Shift10vs.Shift0",
...
cond_means_all <- d_for_analysis %>%
  group_by(Condition.Exposure) %>%
  summarise(across(c(Item.VOT, Item.F0_Mel), mean, .names = "{.col}_mean_all"))

cond_means_exposure <- d_for_analysis %>%
  filter(Phase == "exposure") %>%
  group_by(Condition.Exposure) %>%
  summarise(across(c(Item.VOT, Item.F0_Mel), mean, .names = "{.col}_mean_exposure"))

cond_means <- left_join(cond_means_all, cond_means_exposure)
...
d_for_analysis %<>%
  mutate(
    ...
    VOT.scaled_predicted = intercept.speech_rate + (slope.speech_rate * vowel_duration.scaled),
    VOT.scaled_corrected = (VOT.scaled - VOT.scaled_predicted) + intercept.speech_rate,
...
d_for_analysis %<>%
  mutate(
    ...
    Item.VOT_centered = case_when(
      Condition.Exposure == "Shift0" ~ Item.VOT + (chodroff.mean_VOT - cond_means$Item.VOT_mean_all[1]),
      Condition.Exposure == "Shift10" ~ Item.VOT + (chodroff.mean_VOT - cond_means$Item.VOT_mean_all[2]),
      Condition.Exposure == "Shift40" ~ Item.VOT + (chodroff.mean_VOT - cond_means$Item.VOT_mean_all[3])),
    Item.F0_Mel_centered = case_when(
      Condition.Exposure == "Shift0" ~ Item.F0_Mel + (chodroff.mean_f0_Mel - cond_means$Item.F0_Mel_mean_all[1]),
      Condition.Exposure == "Shift10" ~ Item.F0_Mel + (chodroff.mean_f0_Mel - cond_means$Item.F0_Mel_mean_all[2]),
      Condition.Exposure == "Shift40" ~ Item.F0_Mel + (chodroff.mean_f0_Mel - cond_means$Item.F0_Mel_mean_all[3])))
```
...
p.density <-
  ...
  geom_point(
    data = d_for_analysis %>%
      ...
      summarise(across(c(Item.VOT_centered, Item.F0_Mel_centered), mean)),
    mapping = aes(x = Item.VOT_centered, y = Item.F0_Mel_centered, colour = Condition.Exposure, shape = category),
    size = 2,
    alpha = 0.8,
  inherit.aes = F,
  show.legend = T) +
...
shift10_cov_d <- 
  ...
  mutate(covariance = map_dbl(data, ~ cov(.x$Item.VOT_centered, .x$Item.F0_Mel_centered))) %>%
  pull(covariance))[1]
...
shift10_ellipse <- 
  d_for_analysis %>% 
  filter(Phase == "exposure", Condition.Exposure == "Shift10") %>% 
  group_by(category) %>% 
  summarise(across(c(Item.VOT_centered, Item.F0_Mel_centered), list(mean = mean, var = var), .names = "{.col}.{.fn}")) %>% 
  mutate(
    cov = ifelse(category == "/d/", shift10_cov_d, shift10_cov_t), 
    mu = map2(Item.VOT_centered.mean, Item.F0_Mel_centered.mean, ~ c("VOT" = .x, "f0_Mel" = .y)), 
    Sigma = pmap(list(
      Item.VOT_centered.var, Item.F0_Mel_centered.var, cov), ~ matrix(c(..1, ..3, ..3, ..2), 2, 2, dimnames = list(cues, cues)))) %>%
