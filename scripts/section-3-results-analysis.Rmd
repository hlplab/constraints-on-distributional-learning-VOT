...
cond_means_test_exposure <- d.test_exposure_for_analysis %>% 
  ...
  summarise(across(c(Item.VOT, ), mean, .names = "{.col}_mean_test_exposure"))
  
cond_means_exposure <- d.test_exposure_for_analysis %>% 
  filter(Phase == "exposure") %>% 
  group_by(Condition.Exposure) %>% 
  summarise(across(c(Item.VOT, Item.F0_5ms), mean, .names = "{.col}_mean_exposure"))
    ...
    data = d.test_exposure_for_analysis %>% 
      ...
      mutate(
        ...
        Item.Mel_F0_5ms_centered = case_when(
          Condition.Exposure == "Shift0" ~ Item.Mel_F0_5ms + (chodroff.mean_f0_Mel - cond_means$Item.F0_5ms_mean_test_exposure[1]),
          Condition.Exposure == "Shift10" ~ Item.Mel_F0_5ms + (chodroff.mean_f0_Mel - cond_means$Item.F0_5ms_mean_test_exposure[2]),
          Condition.Exposure == "Shift40" ~ Item.Mel_F0_5ms + (chodroff.mean_f0_Mel - cond_means$Item.F0_5ms_mean_test_exposure[3]))) %>% 
      group_by(Condition.Exposure, category) %>% 
      summarise(across(c(Item.VOT_centered, Item.Mel_F0_5ms_centered), mean)),
    mapping = aes(x = Item.VOT_centered, y = Item.Mel_F0_5ms_centered, colour = Condition.Exposure, shape = category),
    size = 2,
