...
# m.CCuRE <- get_CCuRE_model(d.chodroff_wilson)
...
d_for_analysis %<>%
  ungroup() %>% 
  mutate(VOT.CCuRE = get_CCuRE_VOT(data = d.chodroff_wilson, 
                                   newdata = d_for_analysis %>% ungroup() %>% mutate(VOT = Item.VOT, Talker = "new"))) 
