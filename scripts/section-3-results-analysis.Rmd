...
d.io.categorization <- get_logistic_parameters_from_model(
    io, x, 
    model_col = "io", 
...
d.true_shift <- 
  ...
  mutate(
    ...
    proportion_shift = ifelse(PSE_median >= mean_PSE_block1 & Condition.Exposure != "Shift40", -(proportion_shift), proportion_shift))
...
d.talkerPSEs <- get_IO_categorization(
  ...
  mutate(PSE_centered = PSE - d.true_shift$mean_PSE_block1[1]) %>% 
  summarise(lower_PSE_centered = quantile(PSE_centered, probs = .025), 
            median_PSE_centered = median(PSE_centered),
            upper_PSE_centered = quantile(PSE_centered, probs = .975))
...
p.PSE_1to7 <-     
  ...
  annotate(
    "crossbar",
    x = 9.7,
    y = d.true_shift$mean_PSE_block1[1] + d.talkerPSEs$median_PSE_centered,
    ymin = d.true_shift$mean_PSE_block1[1] + d.talkerPSEs$lower_PSE_centered,
    ymax = d.true_shift$mean_PSE_block1[1] + d.talkerPSEs$upper_PSE_centered,
    width = 0.1,
    colour = "black",
    alpha = .6) +
    coord_cartesian(
