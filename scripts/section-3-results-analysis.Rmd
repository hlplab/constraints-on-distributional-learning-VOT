...
```{r load-format-experimental-data}
...
### Exclusions
Due to data transfer errors 4 participants' data were not stored and therefore not included in this analysis. 
...
## Results
## Regression analysis
The regression analysis addresses several questions: 
...
We fit a Bayesian mixed-effects psychometric model to participants' categorization responses on critical test trials [e.g., @prins2011]. We are primarily interested in the changes in categorization behaviour between test blocks which are presumed to be a consequence of the input from preceding exposure blocks however we fit a separate regression model for exposure in order to visualise participant behaviour during exposure. 
...
```{r model-fit-test-blocks, warning=FALSE}
...
hypotheses <- hypothesis(fit_mix_uniform_bias, paste(rownames(fixef(fit_mix_uniform_bias))[-c(1, 2, 3)], "< 0"))
table <- hypotheses$hypothesis %>% select(-Star)
```


```{r model-fit-exposure-blocks, warning=FALSE}
...
Figure @\ref(fig:plot-fit-intercept-slope-PSE) summarizes participants’ fitted categorization functions across the different test blocks. Of note is the average categorization functions of the respective conditions before exposure informative exposure. As depicted in the first panel, the average categorization functions converge on the same boundary or PSE (45ms, 95% QI = 36ms -- 55ms) which suggests that the three exposure groups largely had similar expectations about the cue distribution corresponding to /d/ and /t/ for this type of talker.  


## Description of the overall pattern of results (main effects)

- The overall lapse rate was negligible ($\hat{\beta} =$ `r make_CI(fit_mix_uniform_bias, "theta1_Intercept ", "theta1_Intercept < 0")`) indicating that participants were paying attention in the majority of trials. 
- There was a main effect of VOT ($\hat{\beta} =$ `r get_CI(fit_mix_uniform_bias, "mu2_VOT_gs", "mu2_VOT_gs > 0")`): participants were more likely to respond "t" as VOT increased. 
...
- Comparing the +10ms condition with the baseline condition across all blocks: there was a reduction in log-odds of responding "t" in the +10ms condition compared to the baseline condition ($\hat{\beta} =$ `r  get_CI(fit_mix_uniform_bias, "mu2_Condition.Exposure_Shift10vs.Shift0", "mu2_Condition.Exposure_Shift10vs.Shift0 < 0")`).
- Comparing the +40ms against the +10ms condition across all blocks: there was a reduction in log-odds of responding "t" in the +40ms condition compared to the +10ms condition ($\hat{\beta} =$ `r  get_CI(fit_mix_uniform_bias, "mu2_Condition.Exposure_Shift40vs.Shift10", "mu2_Condition.Exposure_Shift40vs.Shift10 < 0")`).
- Tellingly, the reduction in log-odds was larger in the +40 vs +10ms comparison, reflecting the larger magnitude of shift from the baseline (`r get_bf(fit_mix_uniform_bias, "mu2_Condition.Exposure_Shift40vs.Shift10 < mu2_Condition.Exposure_Shift10vs.Shift0 ")`).
...
- Comparing the change in differences between +10ms and baseline between blocks: we see an overall reduction in the log-odds of responding "t" between test blocks 1 and 4 however almost all that reduction took place between test blocks 1 and 2 ($\hat{\beta} =$ `r get_CI(fit_mix_uniform_bias, "mu2_Condition.Exposure_Shift10vs.Shift0:Block_Test2vs.Test1", "mu2_Condition.Exposure_Shift10vs.Shift0:Block_Test2vs.Test1 < 0")`). Between test blocks 2 and 4, differences in behaviour between the two groups did not change significantly in spite of increased input from the exposure blocks. 
- Comparing the change in differences between +40ms and +10ms between blocks: 
  -There was a consistent reduction in log-offs of responding "t" from blocks 1 through 4, indicating an incremental shift in categorisation towards the right as participants received more input. The biggest change was observed between test block 1 and 2 ($\hat{\beta} =$ `r get_CI(fit_mix_uniform_bias, "mu2_Condition.Exposure_Shift40vs.Shift10:Block_Test2vs.Test1", "mu2_Condition.Exposure_Shift40vs.Shift10:Block_Test2vs.Test1 < 0")`). 
  - The difference between condition +40 and +10 continued to widen after the second exposure block,  ($\hat{\beta} =$ `r get_CI(fit_mix_uniform_bias, "mu2_Condition.Exposure_Shift40vs.Shift10:Block_Test3vs.Test2", "mu2_Condition.Exposure_Shift40vs.Shift10:Block_Test3vs.Test2 < 0")`) but not much incremental shift was observed in the 4th test block in spite of full exposure to the 144 trials at this stage ($\hat{\beta} =$ `r get_CI(fit_mix_uniform_bias, "mu2_Condition.Exposure_Shift40vs.Shift10:Block_Test4vs.Test3", "mu2_Condition.Exposure_Shift40vs.Shift10:Block_Test4vs.Test3 < 0")`)
...
hyp_interactions_readable <- tibble(Hypothesis = c(
  "diff in 10 vs baseline test 2 > diff in 10 vs baseline test 1",
  "diff in 10 vs baseline test 3 > diff in 10 vs baseline test 2",
  "diff in 10 vs baseline test 4 > diff in 10 vs baseline test 3",
  "diff in 10 vs baseline test 4 vs test 1 < 0",
  "diff in 40 vs 10 test 2 > diff in 40 vs 10 test 1",
  "diff in 40 vs 10 test 3 > diff in 40 vs 10 test 2",
  "diff in 40 vs 10 test 4 > diff in 40 vs 10 test 3",
  "diff in 40 vs 10 test 4 vs test 1 < 0"
))

hyp_interactions <- hypothesis(fit_mix_uniform_bias, hyp_interactions)
...
table1 %>% 
  ...
  rename_all(~ gsub("\\.", " ", .)) %>% 
  kbl(caption = "Comparing interactions of block and condition", align = align_tab(table1)) %>% 
  kable_styling(full_width=FALSE, 
                bootstrap_options = c("striped", "hover", "condensed")) %>% 
  column_spec(1, width = "8em")


# get hypothesis for simple effects of condition by block
hypotheses_contrasts_within_block <- 
  c(
    "mu2_Block1:Condition.Exposure_Shift10vs.Shift0 > mu2_Block1:Condition.Exposure_Shift40vs.Shift10", # diff. between 10 and baseline > diff between +40 and +10 in test 1 
    "mu2_Block3:Condition.Exposure_Shift40vs.Shift10 < mu2_Block3:Condition.Exposure_Shift10vs.Shift0", # shift in 40vs10 > 10 vs. baseline in test 2 
    "mu2_Block5:Condition.Exposure_Shift40vs.Shift10 < mu2_Block5:Condition.Exposure_Shift10vs.Shift0", # shift in 40vs10 > 10 vs baseline in test 3 
    "mu2_Block7:Condition.Exposure_Shift40vs.Shift10 < mu2_Block7:Condition.Exposure_Shift10vs.Shift0") # shift in 40vs 10 > 10 vs 0 within test 4

hypotheses_readable <- tibble(Hypothesis = c(
  "10 vs baseline > diff btwn 40 vs 10 in test 1",
  "diff in 40 vs 10 > diff in 10 vs baseline in test 2",
  "diff in 40 vs 10 > diff in 10 vs baseline in test 3",
  "diff in 40 vs 10 > diff in 10 vs baseline in test 4"
))

hypotheses_contrasts_within_block <- hypothesis(fit_mix_test_nested_block, hypotheses_contrasts_within_block)
hypotheses_contrasts_within_block <- hypotheses_contrasts_within_block$hypothesis %>% 
  select(-Star)

table2 <- cbind(hypotheses_readable, hypotheses_contrasts_within_block) %>% 
  select(-2)

table2 %>% 
  mutate(across(where(is.numeric), ~ round(., digits = 3))) %>% 
  rename_all(~gsub("\\.", " ", .)) %>% 
  kbl(caption = "Comparing conditions within blocks 1 to 4", align = align_tab(table2)) %>% 
  kable_styling(full_width=FALSE, 
                bootstrap_options = c("striped", "hover", "condensed")) %>% 
  column_spec(1, width = "8em")

# hypotheses to analyse last 3 test blocks
hyp_test456 <- c(
...
hyp_test456 <- hypothesis(fit_mix_uniform_bias, hyp_test456) 


hyp_test456_block <- c(
  "mu2_Block8:Condition.Exposure_Shift10vs.Shift0 < mu2_Block7:Condition.Exposure_Shift10vs.Shift0",
  "mu2_Block9:Condition.Exposure_Shift10vs.Shift0 < mu2_Block8:Condition.Exposure_Shift10vs.Shift0 ",
  "mu2_Block8:Condition.Exposure_Shift40vs.Shift10 < mu2_Block7:Condition.Exposure_Shift40vs.Shift10",
  "mu2_Block9:Condition.Exposure_Shift40vs.Shift10 < mu2_Block8:Condition.Exposure_Shift40vs.Shift10")



hyp_test456_block <- hypothesis(fit_mix_test_nested_block, hyp_test456_block)
```
...
```{r expectation-values-exposure-test, message=FALSE, results='hide'}
...
```{r fit-nested-models-for-intercepts-slopes, warning=FALSE, message=FALSE}
# nested models for extracting the 'intercepts' and 'slopes' for each Block and Condition

# nested model for test
...
# nested model for condition:block
...
```{r summarising-draws-from-nested-models, warning=FALSE, message=FALSE}
...
(ref:plot-fit-slope-PSE) **A:** Fitted lapse-rate corrected psychometric plots by exposure condition (all exposure and first 4 test blocks); point ranges indicate the mean proportion of "t"-responses and their 95% bootstrapped CI. **B:** Change in final three test blocks in the absence of more input. **C \& D:** Changes in intercepts, slopes and categorisation boundary (represented by the point-of-subjective-equality (PSE)) by block. Summary is of 8000 draws from the maximum *a posteriori* estimate. Points represent the mean of posterior draws and line ranges are the 95% quantile interval of all draws. Dashed lines show the predicted intercepts, slopes and PSEs by the ideal observers of the respective conditions that have perfectly learned the exposure distributions. 
