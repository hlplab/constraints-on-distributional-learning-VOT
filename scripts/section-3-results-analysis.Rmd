...
(ref:plot-IA-human-PSE) Comparison of shifts in listener PSEs over the first four test blocks against that expected by an idealized adaptor (an idealized learner who adapts to talkers by integrating cue statistics of the exposure talker with prior beliefs about category cue means and variances). Ribbons indicate the 95% highest posterior density interval
```{r, plot-IA-human-PSE, fig.width=5, fig.height=3}
d.estimates %>%
  filter(Block %in% c(1:7)) %>% 
  ggplot(
    aes(
