...
d.talkerPSEs <-
  ...
  make_IOs_from_phonetic_data(
    ...
    # TO DO: this needs to be changed to all three cues: add , "f0", "vowel_duration"
    # but this will require an adjustment to get_PSE_from_io(), which currently assumes 
    # a univariate IO
    cues = c("VOT"),
  ...
  # Center talker PSEs to mean of prior PSEs
  mutate(PSE_centered = prior.PSE_mean - mean(PSE)) %>%
  group_by(speechstyle) %>%
  summarise(
    expected_PSE.lower = quantile(PSE, probs = .025),
    expected_PSE = median(PSE),
    expected_PSE.upper = quantile(PSE, probs = .975))
...
d.psychometric_intercept.slope.PSE %>%
  ...
  annotate(
    "crossbar",
    x = 7.7, 
    y = d.talkerPSEs[d.talkerPSEs$speechstyle == "isolated", ]$expected_PSE, 
    ymin = d.talkerPSEs[d.talkerPSEs$speechstyle == "isolated", ]$expected_PSE.lower, 
    ymax = d.talkerPSEs[d.talkerPSEs$speechstyle == "isolated", ]$expected_PSE.upper,
    colour = "black",
    width = 0.1,
    alpha = .6) +
  annotate(
    "crossbar",
    x = 7.85, 
    y = d.talkerPSEs[d.talkerPSEs$speechstyle == "connected", ]$expected_PSE, 
    ymin = d.talkerPSEs[d.talkerPSEs$speechstyle == "connected", ]$expected_PSE.lower, 
    ymax = d.talkerPSEs[d.talkerPSEs$speechstyle == "connected", ]$expected_PSE.upper,
    colour = "black",
    width = 0.1,
    alpha = .6) +
...
psych.model_prediction <-
  epred_draws(
    ...
    ndraws = 1000) %>%
  ungroup() %>%
  # collapse all observations in block 1 into 1 condition  
  mutate(
  ...
  summarise(proportion_voiceless = mean(Response.Voiceless),
...
There is, however, an alternative explanation that does not throw out the proverbial baby with the bathwater. Under this alternative explanation, adaptive speech perception *is* driven by distributional learning, just not *unconstrained* distributional learning. To appreciate this distinction, it is helpful to refer to a distinction discussed by Xie and colleagues [-@xie2018, p. 2028-2029], the difference between *model learning* and *model selection*. Model learning refers to the idea that listeners learn new phonetic category representations for the unfamiliar talker. This is the idea implemented in the ideal adaptor *model*---not to be confused with the more general *theory*---that we used above to derive the prediction in Figure \@ref(fig:plot-IA-human-PSE). Similarly, the idea that listeners store speech episodes [@goldinger1998; @goldinger2007] or exemplars [@hay2018; @johnson1997] from the unfamiliar talker, and then use these to categorize subsequent speech from that talker is a form of model learning. Critically, as such learning continues, listeners should increasingly come to reflect categorization behavior that is based on the phonetic distributions of the new talker---contrary to what we seem to observer. 
