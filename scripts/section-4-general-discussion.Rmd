...
prior.PSE_mean <- d_IO.intercept_slope_PSE %>%
...
d_psychometric.intercept_slope_PSE %>%
  ...
  geom_linerange(linewidth = .6, position = position_dodge(.3), alpha = .5) +
  geom_hline(
    data = d_IO.intercept_slope_PSE %>% filter(Condition.Exposure %in% c("Shift0", "Shift10", "Shift40"), Block == 7),
...
predictedPSE_40 <- d_IO.intercept_slope_PSE %>%
  select(Condition.Exposure, Block, intercept_scaled) %>%
  filter(Condition.Exposure == "Shift40", Block == 7) %>% pull(intercept_scaled)
predictedPSE_10 <- d_IO.intercept_slope_PSE %>%
  select(Condition.Exposure, Block, intercept_scaled) %>%
  filter(Condition.Exposure == "Shift10", Block == 7) %>% pull(intercept_scaled)
predictedPSE_0 <- d_IO.intercept_slope_PSE %>%
  select(Condition.Exposure, Block, intercept_scaled) %>%
  filter(Condition.Exposure == "Shift0", Block == 7) %>% pull(intercept_scaled)

hyp.shrinkage <-
  hypothesis(
    fit_test_nested_within_condition_and_block,
    c(
      str_c("abs(mu2_IpasteCondition.ExposureBlocksepEQxShift40x1 - mu2_IpasteCondition.ExposureBlocksepEQxShift40x7)/ (mu2_IpasteCondition.ExposureBlocksepEQxShift40x1 -", predictedPSE_40, ") > abs(mu2_IpasteCondition.ExposureBlocksepEQxShift10x1 - mu2_IpasteCondition.ExposureBlocksepEQxShift10x7) / (mu2_IpasteCondition.ExposureBlocksepEQxShift10x1 -", predictedPSE_10, ")"),
      str_c("abs(mu2_IpasteCondition.ExposureBlocksepEQxShift0x1 - mu2_IpasteCondition.ExposureBlocksepEQxShift0x7) / (mu2_IpasteCondition.ExposureBlocksepEQxShift0x1 -", predictedPSE_0, ") < abs(mu2_IpasteCondition.ExposureBlocksepEQxShift10x1 - mu2_IpasteCondition.ExposureBlocksepEQxShift10x7) / (mu2_IpasteCondition.ExposureBlocksepEQxShift10x1 -", predictedPSE_10, ")"),
      str_c("abs(mu2_IpasteCondition.ExposureBlocksepEQxShift0x1 - mu2_IpasteCondition.ExposureBlocksepEQxShift0x7) / (mu2_IpasteCondition.ExposureBlocksepEQxShift0x1 -", predictedPSE_0, ") < abs(mu2_IpasteCondition.ExposureBlocksepEQxShift40x1 - mu2_IpasteCondition.ExposureBlocksepEQxShift40x7)/ (mu2_IpasteCondition.ExposureBlocksepEQxShift40x1 -", predictedPSE_40, ")")),
    robust = T) %>%
  .$hypothesis %>%
  dplyr::select(-Star)
...
make_hyp_table(
    hyp.shrinkage,
    c(
      "$\\Delta$$PSE_{+40}/$$\\Delta$$PSE_{+40_{predicted}} >$ \\\\ $\\Delta$$PSE_{+10} /$ $\\Delta$ $PSE_{+10_{predicted}}$",
      "$\\Delta$$PSE_{baseline} /$$\\Delta$ $PSE_{baseline_{predicted}} <$ \\\\ $\\Delta$$PSE_{+10} /$ $\\Delta$ $PSE_{+10_{predicted}}$",
      "$\\Delta$$PSE_{baseline} /$$\\Delta$ $PSE_{baseline_{predicted}} <$ \\\\ $\\Delta$$PSE_{+40} /$ $\\Delta$ $PSE_{+40_{predicted}}$"),
    ...
    col1_width = "20em") %>%
  pack_rows("Test block 4: Change in PSE as a proportion of predicted change", 1, 3)
```
