...
prior.PSE_mean <- d.IO_intercept.slope.PSE %>%
...
d.talkerPSEs <-
  ...
  mutate(
    center_constant = prior.PSE_mean - mean(PSE),
...
d.psychometric_intercept.slope.PSE %>%
  ...
  geom_linerange(linewidth = .6, position = position_dodge(.3), alpha = .5) +
  geom_hline(
    data = d.IO_intercept.slope.PSE %>% filter(Condition.Exposure %in% c("Shift0", "Shift10", "Shift40"), Block == 7),
...
IA.prediction <-
  ...
  left_join(
    get_test_data_from_stanfit(m_IA_inferred.VOT_f0) %>% get_untransform_function_from_stanfit(m_IA_inferred.VOT_f0)(.)
  ) %>% mutate(
      Response = `/t/` / (`/d/` + `/t/`),
