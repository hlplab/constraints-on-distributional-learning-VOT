...
fit_test_nested <- readRDS("../models/test-nested_slope-priorSD15-0.995.rds")
hyp.shrinkage <-
  hypothesis(
    fit_test.simple_effects_condition,
    c(
      "abs(-6.4 - mu2_Block7:Condition.Exposure_Shift40vs.Shift10) > 
      abs(0.31 - mu2_Block7:Condition.Exposure_Shift40vs.Shift10)",
      "abs(2.7 - mu2_Block7:Condition.Exposure_Shift10vs.Shift0) > 
      abs(0.31 - mu2_Block7:Condition.Exposure_Shift10vs.Shift0)"),
    robust = T) %>%
  .$hypothesis %>%
  dplyr::select(-Star)
...
make_hyp_table(
  hyp.shrinkage,
  c("$PSE_{+40_{expected}} - PSE_{+40} vs. PSE_{+10} > PSE_{+10_{expected}} - PSE_{+40} vs. PSE_{+10}$",
    "$PSE_{baseline_{expected}} - PSE_{+10} vs. PSE_{baseline} > PSE_{+10_{expected}} - PSE_{+10} vs. PSE_{baseline}$"),
