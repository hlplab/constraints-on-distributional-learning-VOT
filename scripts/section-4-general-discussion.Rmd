...
```{r plot-IA-exposure-accuracy, fig.width=6.5, fig.height=3.5, fig.cap="(ref:plot-IA-human-accuracy)"}
d.IA_exposure_accuracy %>% 
  pivot_longer(
...
io.accuracy <- 
  ...
  filter(., Condition.Exposure == "prior") %>% 
      # make 3 rows of prior condition then join each prior with the three exposure conditions
      mutate(rows = 3) %>% 
      uncount(rows) %>% 
...
# calculate human accuracy on unlabelled exposure trials
p.human_accuracy <- 
  d.for_analysis %>%
...
p1 <- p.human_accuracy +
  geom_hline(
    ...
    alpha = 0.4,
    inherit.aes = F) +
  geom_pointrange(position = position_dodge2(.3), size = .6) +
