...
```{r}
# fit a reduced psychometric model of human data for LOO comparability with the IA_predicted logodds model
...
fit_test.upto.test4 <- 
  brm(
    ...
    file = "../models/test-standard-SD15-upto-block4.rds",
...
IA.logodds_change.mean = mean(d.IA_predicted_logodds$IA.logodds_change)
IA.logodds_change.SD = sd(d.IA_predicted_logodds$IA.logodds_change)
IA.logodds_prior.mean = mean(d.IA_predicted_logodds$IA.logodds_prior)
IA.logodds_prior.SD = sd(d.IA_predicted_logodds$IA.logodds_prior)
...
d.IA_predicted_logodds %<>%
  mutate(
    IA.logodds_change_gs = (IA.logodds_change - IA.logodds_change.mean) / (2 * IA.logodds_change.SD),
    IA.logodds_prior_gs = (IA.logodds_prior - IA.logodds_prior.mean) / (2 * IA.logodds_prior.SD),
...
fit_test.IA_predicted_logodds.prior.change.interaction <-
  brm(
    bf(
      Response.Voiceless ~ 1 + (Block_gs * IA.logodds_change_gs) + IA.logodds_prior_gs +
        (1 | ParticipantID) + (1 | Item.MinimalPair)),
    data = d.IA_predicted_logodds,
    prior = priors_logodds_model,
    cores = 4,
...
fit_test.IA_predicted_logodds.prior.change.interaction <- add_criterion(fit_test.IA_predicted_logodds.prior.change.interaction, criterion = c("loo"), moment_match = T)
loo_compare(fit_test.upto.test4, fit_test.IA_predicted_logodds.prior.change, fit_test.IA_predicted_logodds.prior.change.interaction)
...
``` {r fit-IA-predicted-logodds-intercept-slope, eval=FALSE}
...
```{r fit-IA-predicted-logodds-nested, eval=FALSE}
...
fit_test.IA_predicted_logodds_nested <-
  brm(
    bf(
      Response.Voiceless ~ 1,
      mu1 ~ 0 + offset(0),
      mu2 ~ 1 + Block / IA.logodds_change_gs + logodds_prior_gs +
