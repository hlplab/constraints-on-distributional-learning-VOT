...
Here we took steps towards addressing these two requirements. We assessed how well a model of distributional learning, the ideal adaptor, can account for the incremental changes in listeners' perception, across different types and amount of exposure. Our design resulted in 36 exposure combinations that differed in the amount of exposure and the specific phonetic distributions listeners experienced. By measuring listeners' categorization behavior at 12 locations along the phonetic continuum, we were able to assess the ideal adaptor's predictions against listeners' behavior for over 400 unique measurements. Compared to the most common approach---comparisons of the mean accuracy (or proportion of say, ``t''-responses) between just 2 conditions---this constitutes a drastic increase in the resolution of our data. It is exactly such fine-grained data that can facilitate meaningful quantitative assessments of existing models: often, many types of models---and many parameterizations of the same model---can correctly capture any particular aspect of human behavior [@xie2023]. Therefore, the more we design our experiments to elicit data that cover a wider range of the human behavior we seek to understand, the more future experiments can narrow down the space of possible mod 
