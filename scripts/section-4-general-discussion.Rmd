...
(ref:plot-IA-human-PSE) Comparison of shifts in listener PSEs over the first four test blocks against that expected by an ideal adaptor fit to listeners' responses [a distributional learning model that describes ideal information integration, @kleinschmidt-jaeger2015]. Ribbons indicate the 95% highest posterior density interval.
...
```{r evaluate-IA-accuracy}
d.IA_predicted_exposure <-
  if (RESET_MODELS) {
      get_IBBU_predicted_response(
        model = m_IA_inferred.VOT_f0_vowelduration,
        groups = get_group_levels_from_stanfit(m_IA_inferred.VOT_f0_vowelduration),
        untransform_cues = T,
        target_category = 2, # target category "/d/" = 1, "/t/" = 2
        predict_test = F)
    
  saveRDS(d.IA_predicted_exposure, "../models/d.IA_predicted_exposure.rds")
  } else {
    readRDS("../models/d.IA_predicted_exposure.rds")
  }

d.IA_exposure_accuracy <- 
  d.IA_predicted_exposure %>% 
  group_by(group, Condition.Exposure, .chain, .iteration, .draw) %>% 
  summarise(criterion.accuracy = mean(Response.Criterion == category)) %>% 
  right_join(d.IA_predicted_exposure %>% 
  group_by(group, Condition.Exposure, .chain, .iteration, .draw) %>% 
  summarise(proportion.accuracy = mean(Response.Proportion))) %>% 
  group_by(group, Condition.Exposure) %>% 
  summarise(
    across(c(criterion.accuracy, proportion.accuracy), median_hdci))
```

```{r plot-IA-exposure-accuracy, fig.width=6.5, fig.height=3.5, fig.cap="(ref:plot-IA-exposure-accuracy)"}
d.IA_exposure_accuracy %>% 
  pivot_longer(
    cols = contains(".accuracy"),
    cols_vary = "slowest", 
    names_to = "decision_rule") %>% 
  unnest(value) %>% 
  ggplot(
    aes(x = group, y = y, ymin = ymin, ymax = ymax, group = decision_rule, shape = decision_rule, colour = Condition.Exposure)) +
  geom_pointrange(position = position_dodge2(.3)) +
  labs(y = "Median accuracy", x = "IA Group") +
  scale_colour_manual("Condition", values = colours.condition, labels = c("baseline", "+10", "+40")) +
  scale_shape_manual("Decision", values = c(1, 2), labels = c("criterion", "proportion")) +
...
io.accuracy <- 
  ...
  group_by(Condition.Exposure) %T>% 
  { summarise(., accuracy.proportional = mean(categorization.proportional_response)) ->> temp } %>% 
  summarise(., accuracy.criterion = mean(categorization.criterion_response == x_intended.category)) %>% 
  right_join(temp) %>% 
  bind_rows(
    io %>% 
  select(-c(fold, x)) %>% 
...
# get of the fitted psychometric model
fit_exposure <- readRDS("../models/exposure-standard-priorSD15-0.999.rds")
psych.model_accuracy <- epred_draws(object = fit_exposure,
  ...
  mutate(proportional.rule = ifelse(category == "/t/", .epred, 1 -.epred),
         criterion.rule = case_when(category == "/t/" & .epred >= .5 ~ 1,
                                    category == "/d/" & .epred < .5 ~ 1,
                                    T ~ 0)) %>% 
  summarise(across(c(proportional.rule, criterion.rule), mean, .names = "{.col}")) %>% 
  group_by(Condition.Exposure, Block) %>% 
  median_hdci(proportional.rule, criterion.rule)
...
p.human_accuracy <- 
  ...
  relabel_blocks() %>% 
  ggplot(aes(x = Block.plot_label, y = median, ymin = lower, ymax = upper, colour = Condition.Exposure)) 
```

(ref:plot-IO-human-accuracy) Range of mean accuracies of participants during exposure phases against accuracies of idealised learners assumed to have perfectly learned the exposure distributions. Point ranges represent the median and 95% quantile interval of participant means. **Top:** horizontal lines are accuracies of ideal observers trained in the respective conditions. **Bottom:** Same as top but ideal observer lines represent their recognition accuracy prior to receiving any exposure from the respective exposure conditions.
```{r plot-io-human-accuracy, fig.width=6.5, fig.height=5.5, fig.cap="(ref:plot-IO-human-accuracy)"}
p1 <- p.human_accuracy +
  geom_hline(
    data = io.accuracy %>% 
      filter(Condition.io != "prior"),
    mapping = aes(yintercept = accuracy, color = Condition.Exposure, linetype = decision),
    linewidth = .8,
    alpha = 0.4,
    inherit.aes = F) +
  geom_pointrange(position = position_dodge2(.3), size = .4) +
   labs(y = "Human accuracy on unlabeled exposure trials") +
...
p2 <- p.human_accuracy +
  geom_hline(
    ...
    inherit.aes = F) +
  geom_pointrange(position = position_dodge2(.3), size = .4) +
  labs(x = "Block", y = "Human accuracy on unlabeled exposure trials") +
  scale_colour_manual("Condition", values = colours.condition, labels = c("baseline", "+10", "+40"), guide = F) 
...
## Limitations and future directions
 ...
 + scaling ecological validity (repeat modest progress we made). As this is the first, or one of the first, attempts to address this question, we intentionally focused on a simple two-way phonetic contrast, sacrificing ecological validity for experimental control.
