...
p.gaussian <- 
  ggplot() +
  d.talker_IO$gaussian + 
  (d.talker_types %>% 
  filter(Talker == "average") %>% .$gaussian) +
  (d.talker_types %>% 
  filter(Talker == "shifted") %>% .$gaussian) +
  scale_colour_manual("Category", values = colours.category_greyscale) +
...
d.talker_cat <- 
  ...
  mutate(rows = 2) %>% 
  uncount(rows) %>% 
  mutate(Talker = factor(rep(c("average", "shifted"), 2)),
                Shift = rep(c(0, 32), 2),
         VOT = VOT + Shift) %>%  
  select(-Shift) %>% 
  group_by(Talker) %>% 
  nest(data = -Talker) %>% 
  mutate(io = map(
    data, ~ make_MVG_ideal_observer_from_data(
              data = .x,
              cues = c("VOT"),
              Sigma_noise = matrix(c(120), ncol = 1, dimnames = list(c("VOT"), c("VOT"))),
      prior = c("/d/" = .5, "/t/" = .5),
      lapse_bias = c("/d/" = .5, "/t/" = .5)) %>% 
      mutate(Sigma = ifelse(category == "/d/", 10, 35)))) %>% 
  left_join(
    tibble(Talker = rep(c("average", "shifted"), 2)) %>% 
           expand_grid(x = seq(-25, 110, .5)) %>% 
      group_by(Talker) %>% 
      nest(x = x)) %>% 
  mutate(
     categorization =
...
arrow_PSE <- 
  d.talker_types %>% 
  filter(Talker == "shifted") %>% 
  mutate(x = PSE, xend = PSE, y = .5, yend = .01) 

p.cat <- 
  d.talker_cat %>% 
  select(Talker, categorization) %>% 
  group_by(Talker) %>% 
  unnest(categorization, names_repair = "unique") %>% 
  ggplot(aes(x = VOT, y = response, group = Talker, colour = Talker, linetype = Talker)) +
  geom_line(linewidth = 1.2) +
  scale_colour_manual(values = c("#b4dafe", "#02427e")) +
  scale_linetype_manual(values = c("solid", "longdash")) +
  scale_x_continuous("VOT (ms)", limits = c(-25, 130), breaks = c(0, 50, 72, 100)) +
  scale_y_continuous("Proportion 't'-response", breaks = c(0, .5, 1)) +
  guides(linetype = "none", colour = "none") +
  geom_segment(data = arrow_PSE,
...
p.PSE <- 
  ...
  geom_point(size = 1.5) +
  scale_colour_manual(values = c("#b4dafe", "#02427e"), guide = "none") + 
  scale_x_discrete("Exposure", labels = c("prior", "post")) +
  scale_y_continuous("PSE (ms VOT)", limits = c(20, 75), breaks = c(37, 72))
  
 
PSE_change <- expand_grid(
    PSE = c(),
    learning_pattern = factor(c("immediate", "linear", "diminished", "plateau")),
    exposure_amount = factor(c("0", "1/3", "2/3", "all"))) %>%
  mutate(learning_pattern = fct_relevel(learning_pattern, "immediate", "linear", "diminished", "plateau"),
         exposure_amount = fct_relevel(exposure_amount, "0", "1/3", "2/3", "all"),
         PSE = case_when(exposure_amount == "0" ~ 37,
                         exposure_amount == "all" & learning_pattern != "plateau" ~ 72,
                         exposure_amount == "1/3" & learning_pattern == "immediate" ~ 72,
                         exposure_amount == "2/3" & learning_pattern == "immediate" ~ 72,
                         exposure_amount == "1/3" & learning_pattern == "linear" ~ 37 + ((72 - 37 + 1)/3),
                         exposure_amount == "2/3" & learning_pattern == "linear" ~ 37 + 12 + ((72 - 37 + 1)/3),
                         exposure_amount == "1/3" & learning_pattern == "diminished" ~ 37 + ((72 - 37 + 1)/2),
                         exposure_amount == "2/3" & learning_pattern == "diminished" ~ 37 + 16 + ((72 - 37 + 1)/3),
                         exposure_amount == "1/3" & learning_pattern == "plateau" ~ 37 + ((72 - 37 + 1)/2),
                         exposure_amount == "2/3" & learning_pattern == "plateau" ~ 37 + 18 + ((72 - 37 + 1)/3),
                         exposure_amount == "all" & learning_pattern == "plateau" ~ 37 + 18 + ((72 - 37 + 1)/3)))
...
p.PSE_change <- 
  ...
  geom_line(alpha = .5) +
  geom_hline(aes(yintercept = 72), linetype = 2, alpha = .6, colour = "grey") +
  scale_x_discrete("Exposure amount", labels = c("prior", "1/3 ", "2/3", "all")) +
  scale_y_continuous("PSE", limits = c(25, 75), breaks = c(30, 40, 50, 60, 70)) +
  guides(alpha = "none") +
  facet_wrap(~learning_pattern, nrow = 1)
...
<!-- TO DO: translate into knitr table so that the references work -->
\begin{table}[!ht]
\begin{small}
\begin{tabular}{p{0.4\textwidth}p{0.55\textwidth}}
\hline
Prediction 1 ({\bf prior expectations}) & AA, [Kang \& Schertz (2021); Schertz et al. (2016); Xie et al. (2021); Tan et al. (2021)] \\

Prediction 2a ({\bf exposure amount})  & LGPL/VGPL, [Vroomen et al., (2007); Cummings \& Theodore (2023); Kleinschmidt \& Jaeger (2011); Liu \& Jaeger (2018)] \\

Prediction 2b ({\bf exposure distribution}) & AA, [Hitczenko \& Feldman (2016); Xie et al. (2021), Tan et al. (2021)]; DL, [Clayards et al. (2008), Idemaru \& Holt (2011); Kleinschmidt \& Jaeger (2016); Chladkova et al. (2017); Colby et al. (2018), Theodore \& Monto (2019)] \\
