...
```{r kleinschmidt-jaeger-2016-replotted, fig.height=base.height*2+1/2, fig.width=base.width*4, fig.cap="(ref:kleinschmidt-jaeger-2016-replotted)"}
...
p.KJ16.histogram <-
  ...
  ggplot(aes(x = Item.VOT,
             ...
             colour = paste(Condition.Exposure, trueCat))) +
  geom_histogram(binwidth = 5) +
  scale_x_continuous("VOT (msec)", breaks = seq(-50, 150, 50)) +
  ...
  scale_colour_manual(
   ...
   guides(
    colour = guide_legend(
      override.aes = list(
        colour = c("#383838", "#C0C0C0", 0, 0, 0, 0, 0, 0),
        fill = c("#383838", "#C0C0C0", 0, 0, 0, 0, 0, 0),
        values = c("b", "p", 0, 0, 0, 0, 0, 0)), nrow = 1)) +
...
p.KJ16.result <- 
  ...
  geom_ribbon(aes(
    ...
    y = estimate__,
    group = Condition,
    ymin = lower__, ymax = upper__, fill = Condition), alpha = .1) +
  geom_line(aes(
    x = Item.VOT,
    y = estimate__,
    group = Condition,
...
(p.KJ16.histogram / p.KJ16.result) +
...
>>>>>>> b4fb8b690e7ea047d8472121be1eb94925f0da7f
