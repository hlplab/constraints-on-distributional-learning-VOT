...
# TO DO: this needs fixing. It also looks like the categorizations have already been obtained and are stored in
# d.talker_IO.VOT. So there would seem to be no need to repeat all of that here!
d.talker_cat <- 
  ...
  mutate(rows = 2) %>% 
  uncount(rows) %>% 
  mutate(Talker = factor(rep(c("typical", "unfamiliar"), 2)),
  ...
  left_join(
    tibble(Talker = rep(c("typical", "unfamiliar"), 2)) %>% 
...
\hline
Prediction 1 ({\bf prior expectations}) & (Kang \& Schertz, 2021; Schertz et al., 2016; Tan et al., 2021; Xie et al., 2021) \\

Prediction 2a ({\bf exposure amount})  & (Vroomen et al., 2007; Cummings \& Theodore, 2023; Kleinschmidt \& Jaeger, 2011; Liu \& Jaeger, 2018) \\

Prediction 2b ({\bf exposure distribution}) & (Chl\'adkov\'a et al., 2017; Clayards et al., 2008; Colby et al., 2018; Hitczenko \& Feldman, 2016; Idemaru \& Holt, 2011; Kleinschmidt, 2020; Theodore \& Monto, 2019) \\
...
\hline
\end{tabular}
\caption{Predictions of distributional learning models about incremental adaptation to an unfamiliar talker. Also listed are examples of previous work that provide some initial, almost always qualitative, evidence for these predictions. To the best of our knowledge, prediction (2a) is the only one that has been tested against {\em incremental} changes in listeners' behavior.}
