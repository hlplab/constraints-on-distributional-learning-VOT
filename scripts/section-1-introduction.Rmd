...
p.cat <- 
  ...
  ggplot(aes(x = VOT, y = response, group = Block, colour = Block)) +
  geom_line(
  data = d.cat_demo %>% 
  unnest(categorization, names_repair = "unique") %>% 
  ...
  scale_colour_manual(values = c("#bfbfb3", "#595959")) +
  geom_line(
    data = d.cat_demo %>% 
  unnest(categorization, names_repair = "unique") %>% 
  ...
  colour = "#262626",
  inherit.aes = F) +
  scale_x_continuous("VOT (ms)", limits = c(-25, 130), breaks = c(0, 34, 50, 68, 100)) +
  labs(y = "Response /t/") +
...
p.PSE <- d.cat_demo %>% 
  ...
  geom_point() +
  scale_colour_manual(values = c("#e6e6fc", "#bfbfb3", "#595959", "#262626"), guide = "none") + 
...
Prediction 2a ({\bf exposure amount})  & LGPL/VGPL, \cite{cummings-theodore2023, kleinschmidt-jaeger2011, liu-jaeger2018, vroomen2007} \\
