...
p.gaussian <- 
  ...
  d.talker_average$gaussian +
  d.talker_shifted$gaussian +
...
d.cat_demo <- 
  ...
  mutate(rows = 4) %>% 
  uncount(rows) %>% 
  mutate(Block = factor(rep(c("Block_0", "Block_1", "Block_2", "Block_3"), 2)),
                Shift = rep(c(-5, 15, 25, 29), 2),
         VOT = VOT + Shift) %>%  
  select(-Shift) %>% 
  group_by(Block) %>% 
  nest(data = -Block) %>% 
  mutate(io = map(
    data, ~ make_MVG_ideal_observer_from_data(
              data = .x,
              cues = c("VOT"),
              Sigma_noise = matrix(c(80), ncol = 1, dimnames = list(c("VOT"), c("VOT"))),
      prior = c("/d/" = .5, "/t/" = .5),
      lapse_bias = c("/d/" = .5, "/t/" = .5)) %>% 
      mutate(Sigma = ifelse(category == "/d/", 9.2, 21)))) %>% 
  left_join(
    tibble(Block = rep(c("Block_0", "Block_1", "Block_2", "Block_3"), 2)) %>% 
...
arrow_PSE <- 
  ...
  mutate(response = abs(response - .5)) %>% 
  group_by(Block) %>% 
...
p.cat <- 
  d.cat_demo %>% 
  unnest(categorization, names_repair = "unique") %>% 
  filter(Block %in% c("Block_1", "Block_2")) %>% 
  ggplot(aes(x = VOT, y = response, group = Block, colour = Block), alpha = .6) +
  geom_line(
  data = d.cat_demo %>% 
  unnest(categorization, names_repair = "unique") %>% 
  filter(Block == "Block_0"),
  ...
  scale_colour_manual(values = c("#bfbfb3", "#808080")) +
  geom_line(
    data = d.cat_demo %>% 
  unnest(categorization, names_repair = "unique") %>% 
  filter(Block == "Block_3"),
  mapping = aes(x = VOT, y = response),
  linetype = "longdash",
  linewidth = .9,
  colour = "#262626",
  inherit.aes = F) +
  scale_x_continuous("VOT (ms)", limits = c(-25, 130), breaks = c(0, 34, 54, 68, 100)) +
  labs(y = "Response /t/") +
...
p.PSE <- d.cat_demo %>% 
  ...
  slice_sample(n = 1) %>% 
  rename(PSE = VOT) %>% 
  ggplot(aes(x = Block, y = PSE, colour = Block)) +
  geom_point() +
  scale_colour_manual(values = c("#e6e6e6", "#bfbfb3", "#808080", "#262626"), guide = "none") + 
  scale_x_discrete("Exposure amount", labels = c("0", "1/3 ", "2/3", "all")) +
  scale_y_continuous("PSE", limits = c(20, 70), breaks = c(34, 54, 64, 68))
  
p.gaussian/p.cat | p.PSE
```
