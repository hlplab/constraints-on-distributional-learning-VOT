...
d.for_RT_exclusion <- d.exposure_test %>%
  filter(
    Is.CatchTrial == FALSE &
...
d.for_RT_exclusion %<>% 
  filter(Exclude_participant.due_to_catch_trials == FALSE &
           Exclude_participant.due_to_labeled_trials == FALSE &
           Exclude_participant.due_to_VOT_slope == FALSE) %>% 
...
excl.VOT <- d.exposure_test %>% 
  ...
  group_by(Condition.Exposure, ParticipantID) %>% 
  summarise() %>% 
  tally() %>% rename(VOT.Slope = n)

# table of exclusions
# excl.VOT %>% 
#   full_join(excl.catch) %>% 
#   full_join(excl.labeled) %>% 
#   full_join(excl.VOT) %>% 
#   full_join(excl.RT.participant) %>% 
#   mutate(across(.cols = c(VOT.Slope, Catch, Labeled, RT.Participant), .fns = ~ ifelse(is.na(.x), 0, .x))) 
```
### Exclusions
We excluded from analysis participants who committed more than 3 errors out of the 18 catch trials (<84% accuracy, N = 1), participants who committed more than 4 errors out of the 72 catch trials (<94% accuracy, N = 0), participants with an average reaction time (RT) more than three standard deviations from the mean of the by-participant means (N = 0), and participants who reported not to have used headphones (N = 0) or not to be native (L1) speakers of US English (N = 0). In addition, participants' categorization during the early phase of the experiment were scrutinise for their slope direction and their
For the remaining participants, trials that were more than three SDs from the participant's mean RT were excluded from analysis (`r proportion.excluded * 100 `%). Finally, we excluded participants (N = 0) who had less than 50% data remaining after these exclusions. 
