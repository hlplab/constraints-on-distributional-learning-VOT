...
VOTs = seq(0, 85, .5) + (chodroff.mean_VOT - VOT.mean_exp2) # defining VOTs as temporary debugging measure
...
While there was weak evidence for a main effect of block its interaction with condition revealed how participants in the respective groups responded as they progressively received more informative exposure. Most of the change took place after the first exposure block. There is some evidence that the difference in proportion of "t"-responses between condition +10ms and +0ms in block 3 compared to that in block 1 ($\hat{\beta} =$ `r get_CI(fit_mix_uniform_bias, "mu2_Condition.Exposure_Shift0vs.Shift10:Block_Block1vs.Block3", "mu2_Condition.Exposure_Shift0vs.Shift10:Block_Block1vs.Block3 < 0")`) was meaningful. In block 3, participants in the +10ms condition on average, responded with fewer "ts" *relative* to participants in the +0ms condition. The evidence for the difference in proportion of "t" responses between the  +40ms and +10ms condition in block 3 compared to that in block 1 was much stronger ($\hat{\beta} =$ `r get_CI(fit_mix_uniform_bias, "mu2_Condition.Exposure_Shift10vs.Shift40:Block_Block1vs.Block3", "mu2_Condition.Exposure_Shift10vs.Shift40:Block_Block1vs.Block3 < 0")`). 

[THE WORDING BELOW THAT DESCRIBE THE ESTIMATES OF THE INTERACTIONS COULD BE WRONG -- I'M UNDER THE IMPRESSION THAT THE INTERACTION MEASURES THE DIFFERENCE BETWEEN THE DIFFERENCES IN CONDITIONS FROM 1 BLOCK TO THE NEXT BLOCK -- WHAT DOES A POSITIVE ESTIMATE INDICATE? IS IT THAT THE DIFF IN LOG-ODDS BETWEEN +0 AND +10 IN BLOCK 5 IS LARGER THAN THE DIFF IN LOG-ODDS BETWEEN +0 AND +10 IN BLOCK 3? IT DOESN'T LOOK THAT WAY IN THE PLOTS: see figure PANEL 3 DIFFERENCE BETWEEN +10 AND +0 LOOKS NARROWER THAN PANEL 2]
Moving to test block 5, the difference in average log-odds between +0ms and +10ms in block 5 relative to block 3 was *positive* such that the margin of difference between the two conditions in block 5 was smaller than the corresponding margin in block 3 ($\hat{\beta} =$ `r get_CI(fit_mix_uniform_bias, "mu2_Condition.Exposure_Shift0vs.Shift10:Block_Block3vs.Block5", "mu2_Condition.Exposure_Shift0vs.Shift10:Block_Block3vs.Block5 >0")`). In blocks 7 and 8, the  average log-odds difference between +0ms and +10ms increased marginally (as indicated by the negative signs of the estimates), although evidence The effects in both comparisons was marginally weaker than that seen in test block 3 while subsequent blocks showed attenuating effects of the interaction between condition and test block. 

```{r, results='hide'}
# get fits by condition for plotting
