...
PSE_VOT <- get_PSE_quantiles(data = VOT.no.noise, group = "gender")
...
# center exposure data relative to database and Gelman-scale them
d.block1 %>% 
      mutate(Item.VOT = (Item.VOT + (chodroff.mean_VOT - VOT.mean_norm)),
             VOT_gs = (Item.VOT - chodroff.mean_VOT)/(2 * VOT.sd_norm))

# get expected values of the centered VOT under the model
if (file.exists("../models/centered_exposure_block1_exp2.rds")) {
  centered_block1 <- read_rds("../models/centered_exposure_block1_exp2.rds")
} else {
  centered_block1 <- fit_mix_block1 %>%
    epred_draws(
    newdata = d.block1 %>% 
      mutate(Item.VOT = Item.VOT + (chodroff.mean_VOT - VOT.mean_norm),
             VOT_gs = (Item.VOT - chodroff.mean_VOT)/(2 * VOT.sd_norm),
    ...
    re_formula = NA,
    ndraws = 2000)
  write_rds(centered_exposure, file = "../models/centered_exposure_block1_exp2.rds")
}
# get the expected CI based on the draws 
centered_block1 %<>% 
  group_by(VOT_gs) %>% 
  summarise(
...
centered_block1 %>% 
  ggplot(aes(x = descale(VOT_gs, chodroff.mean_VOT, VOT.sd_norm), 
             y = estimate__)) +
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = .1) +
...
p.IOs.VOT_F0.centered.input <- plot_IO_fit(
  ...
  data.perception = centered_exposure %>% 
    mutate(Item.VOT = descale(VOT_gs, chodroff.mean_VOT, VOT.sd_norm)),
  data.test = d.block1 %>%  
    ungroup() %>% distinct(Item.VOT),
  posterior.sample = post_sample_block1,
  PSE_VOT_F0.centered) 
