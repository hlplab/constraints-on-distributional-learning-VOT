...
p.likelihood.univar_block1 <- d.test.IO_block1 %>% 
  ...
  geom_label(aes(y = mode_likelihood_per_response,
             label = "+"),
             size = 3.5,
...
p.likelihood.multivar_block1 <- d.test.IO_block1 %>% 
  ...
  geom_label(aes(y = mode_likelihood_per_response, label = "+"), 
             size = 3,
             position = position_dodge(.2),
             show.legend = F) +
  geom_errorbar(
...
fit_mix_helmert <- read_rds("../models/Exp-AE-DLVOT-labelled-lapsing-GLMM-helmert.rds")
...
p.PSE <- d.PSE %>% 
  ...
  geom_hline(yintercept = 25, linetype = 2, linewidth = 0.8, colour = "#cc0000", alpha = 0.8) +
  geom_hline(yintercept = 25 + 10, linetype = 2, linewidth = .8, colour = "#12D432", alpha = 0.5) +
  geom_hline(yintercept = 25 + 40, linetype = 2, linewidth = .8, colour = "#0481F3", alpha = 0.5) +
  scale_colour_manual("Condition",
    labels = c("+0ms", "+10ms", "+40ms"),
    values = c("#cc0000", "#12D432","#0481F3"),
    aesthetics = "color") +
  scale_y_continuous("Median PSE (ms)", limits = c(24.5, 65.5), breaks = seq(25, 65.5, 5)) +
...
```{r}
d.temp <- d.test_exposure_for_analysis %>% filter(Condition.Exposure == "Shift0")
