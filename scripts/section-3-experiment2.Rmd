...
psychometric_fit_block1 %>% group_by(VOT_gs) %>% 
  ...
  summarise(across(c(VOT_gs, Item.VOT), range))
```
  ...
  geom_errorbar(
    data = d.bootstrap_block1 %>% 
      ...
      geom_errorbar(
        ...
        width = 0,
        inherit.aes = F) +
     scale_y_continuous("Median likelihood per response", limits = c(.10736, .11005)) + 
     scale_x_discrete( 
...
p.likelihood.multivar_block1 <- d.test.IO_block1 %>% 
  ...
  geom_errorbar(
    ...
    width = 0,
    inherit.aes = F) +
  scale_y_continuous("Median likelihood per response", limits = c(.156, .746)) +
  scale_x_discrete( 
...
cond_fit[[1]] %>%
  mutate(Condition.Exposure = case_when(Condition.Exposure == "Shift0" ~ "+0ms",
    ...
    filter(Block %in% c(1, 3, 5, 7, 8, 9)) %>% 
  mutate(Block_relabelled = factor(case_when(Block == 1 ~ "Test 1",
                                      Block == 3 ~ "Test 2",
                                      Block == 5 ~ "Test 3",
                                      Block == 7 ~ "Test 4",
                                      Block == 8 ~ "Test 5",
  ...
  stat_summary(
    data = d.test_exposure_for_analysis %>% 
            ...
            Condition.Exposure == "Shift10" ~ "+10ms",
            Condition.Exposure == "Shift40" ~ "+40ms"), 
            Block_relabelled = factor(case_when(Block == 1 ~ "Test 1",
                                      Block == 3 ~ "Test 2",
                                      Block == 5 ~ "Test 3",
                                      Block == 7 ~ "Test 4",
                                      Block == 8 ~ "Test 5",
                                      Block == 9 ~ "Test 6"))) %>% 
      group_by(Condition.Exposure, ParticipantID, Block_relabelled), 
