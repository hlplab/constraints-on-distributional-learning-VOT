...
VOTs_centered = seq(0, 85, .5) + (chodroff.mean_VOT - VOT.mean_exp2) 
F0s_centered <- normMel(predict_f0(VOTs_centered)) + (chodroff.mean_f0_Mel - f0.mean_exp2)
...
if (file.exists("../data/d.test.IO_block1.csv")) {
  d.test.IO_block1 <- read_rds("../data/d.test.IO_block1.csv")
} else {
d.test.IO_block1 %<>%
  mutate(
...
d.PSE <- fit_mix_nested %>% 
  ...
  mutate(PSE = descale(-Intercept/slope, VOT.mean_test, VOT.sd_test)) %>% 
...
```{r compare-density-database-with-exposure-means}
...
d.chodroff_wilson.selected %>% 
  ...
  geom_point(
    ...
    mapping = aes(x = Item.VOT_centered, y = Item.Mel_F0_5ms_centered, group = Condition.Exposure, shape = category),
    size = 3,
  inherit.aes = F) +
scale_colour_manual(values = c("/d/" = "#00000", "/t/" = "#808080")) +
scale_y_continuous(limits = c(160, 320)) +
scale_x_continuous("VOT (msec)", limits = c(-2, 95), breaks = seq(0, 95, 5))
...
# use scaled VOT, get intercept and slope from scaled
```
