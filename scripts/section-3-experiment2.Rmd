...
d.ranef %>%
  ...
  geom_text(aes(label = ParticipantID), alpha = .2) +
...
psychometric_fit_block1 %>% group_by(VOT_gs) %>% 
  mutate(Item.VOT = descale(VOT_gs, VOT.mean_exp2, VOT.sd_exp2)) %>% summarise(Item.VOT)
```
...
```{r, message=F, fig.cap="Temporary plot to show the expected fit after centering the cues. "}
...
epred_centered_block1 %>% 
  ...
  ggplot(aes(x = descale(VOT_gs, VOT.mean_exp2, VOT.sd_exp2) - (chodroff.mean_VOT - VOT.mean_exp2), 
             y = estimate__)) +
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = .1) +
  geom_line(colour = "#333333", linewidth = 1.2) +
  scale_x_continuous("VOT (ms)", limits = c(-10, 75)) +
  scale_y_continuous("Fitted proportion of 't'-responses")
```
...
```{r prepare-plots-to-make-IO-plot-grid-block1, fig.width=4, fig.height=8.5}
# make plot grid
remove_all_titles <- 
    ...
    log_likelihood_per_response =
      pmap(
        ...
        ~ get_average_log_likelihood_of_perception_data_under_IO(
          observed_inputs = if(str_detect(..3, "centered.input")) {..1$VOT_F0_centered}
...
p.likelihood_block1 <- d.test.IO_block1 %>% 
  ...
  summarise(
    median_likelihood_per_response = median(likelihood_per_response)) %>%
  mutate(io.type = fct_relevel(io.type, c("VOT.no.noise_block1", "VOT_block1"))) %>% 
  ...
  geom_errorbar(
    ...
    width = 0,
    inherit.aes = F) +
  scale_y_continuous("Median likelihood per response", limits = c(.10, .65)) +
   scale_x_discrete( 
...
p.likelihood.multivar_block1 <- d.test.IO_block1 %>% 
  ...
  geom_errorbar(
    ...
    width = 0,
    inherit.aes = F) +
  scale_y_continuous("Median likelihood per response", limits = c(.5, .76)) +
  scale_x_discrete( 
