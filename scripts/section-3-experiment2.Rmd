...
PSE_VOT.no.noise_block1 <- get_PSE_quantiles(data = VOT.no.noise_block1, group = "gender")
...
VOT.centered.input_block1 <- 
  get_IO_categorization(
    cues = c("VOT_centered"), 
    groups = c("Talker", "gender"), 
    VOTs = seq(0, 85, .5) + (chodroff.mean_VOT - VOT.mean_testblock1),
    F0s = normMel(predict_f0(VOTs)) + (chodroff.mean_f0_Mel - f0.mean_testblock1),
    lapse_rate = lapse_exp2,
    alpha = .1, 
    linewidth = 0.3,
...
p.IOs.VOT.centered.input_block1 <- plot_IO_fit(
  ...
  data.test = d.block1 %>% 
    ...
    distinct(Item.VOT),
  data.percept.PSE = post_sample_block1 %>% 
    mutate(PSE = PSE - (chodroff.mean_VOT - VOT.mean_testblock1),
           .lower = .lower - (chodroff.mean_VOT - VOT.mean_testblock1),
           .upper = .upper - (chodroff.mean_VOT - VOT.mean_testblock1)),
...
```{r IO-categorisation-VOT-F0-exposure-centered, message=FALSE, warning=FALSE}
VOTs_centered = seq(0, 85, .5) + (chodroff.mean_VOT - VOT.mean_testblock1) 
F0s_centered <- normMel(predict_f0(VOTs_centered)) + (chodroff.mean_f0_Mel - f0.mean_testblock1)
...
p.IOs.VOT_F0.centered.input_block1 <- plot_IO_fit(
  ...
  PSE_VOT_F0.centered.input_block1 %>% 
    mutate(PSE.lower = round(PSE.lower - (chodroff.mean_VOT - VOT.mean_testblock1)),
           PSE.median = PSE.median - (chodroff.mean_VOT - VOT.mean_testblock1),
           PSE.upper = round(PSE.upper - (chodroff.mean_VOT - VOT.mean_testblock1)))) 
```



(ref:comparative-production-IO-plot-block1) From top to bottom: description

```{r comparative-production-IO-plot-block1, fig.width=4, fig.height=8.5, warning=FALSE}
# make plot grid
remove_all_titles <- 
...
d.test.IO_block1 <- 
  ...
  mutate(
    ...
    Item.VOT_centered = Item.VOT + (chodroff.mean_VOT - VOT.mean_testblock1),
...
if (file.exists("../data/d.test.IO_block1.csv")) {
  d.test.IO_block1 <- read_csv("../data/d.test.IO_block1.csv")
} else {
d.test.IO_block1 %<>%
  mutate(
    log_likelihood_per_response =
      pmap(
        ...
        ~ get_average_log_likelihood_of_perception_data_under_IO(
          ...
          else if (str_detect(..3, "VOT\\.no\\.noise_block1$")) {..1$Item.VOT}
          else if (str_detect(..3, "VOT\\.centered_block1$")) {..1$Item.VOT}
          else if (str_detect(..3, "VOT_block1$")) {..1$Item.VOT}, 
  ...
  write_csv(d.test.IO_block1, "../data/d.test.IO_block1.csv")
}  
...
```{r likelihood-comparison-plots-block1, fig.width=3.8, fig.height=3.5}
...
IO.likelihood_block1 <- d.test.IO_block1 %>% 
  ...
  mutate(cues = ifelse(str_detect(io.type, "VOT_F0.*block1$"), "VOT-F0", "VOT"))
...
p.likelihood.block1 <- IO.likelihood_block1 %>% 
  ...
  theme(panel.grid.major.x = element_blank(),
        legend.position = "top",
        axis.text.x = element_text(angle = 22.5, vjust = 1, hjust = 1)) + 
...
io.conditions <- crossing(conditions, ios) %>% 
  cbind(tibble(mu = list(c(VOT = exposure_shift0_mean_d), c(VOT = exposure_shift0_mean_t), c(VOT = exposure_shift10_mean_d), c(VOT = 57.9249), c(VOT = 41.1887), c(VOT = 87.9249)))) %>% 
