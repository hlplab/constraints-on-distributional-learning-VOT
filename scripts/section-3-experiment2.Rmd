...
```{r IO-categorization-VOT-f0-centered-input-block1}
...
```{r likelihood-comparison-plots-block1, fig.height=4.5, fig.width=5.5, warning=FALSE, cache=FALSE}
...
cond_fit <- conditional_effects(
    ...
    conditions = make_conditions(
      d.test_exposure_for_analysis %>% 
        filter(Phase == "test") %>% 
        prepVars(levels.Condition = levels_Condition.Exposure, contrast_type = "repeated"), 
      vars = c("Block")),
    method = "posterior_epred",
    ndraws = 500,
    re_formula = NA)
...
# summary = F below gives all the posterior draws
temp <- 
...
d.PSE <- fit_mix_nested %>% 
  ...
  mutate(Block = factor(case_when(Block == 1 ~ "Test 1",
                                Block == 3 ~ "Test 2",
                                Block == 5 ~ "Test 3",
                                Block == 7 ~ "Test 4",
                                Block == 8 ~ "Test 5",
