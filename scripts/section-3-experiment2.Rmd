...
lapse_exp2 = plogis(as.numeric(summary(fit_mix_uniform_bias)$fixed["theta1_Intercept", 1]))
...
# join production IOs with perception data
d.test.IO_block1 <- 
  d.block1 %>%
  ungroup() %>%
...
# calculate the likelihood of each IO under the data
d.test.IO_block1 %<>%
  mutate(
...
# use bootstrap samples by io type to find the CI of the median and mode
set.seed(7896)
...
```{r likelihood-comparison-plots-block1, fig.width=6.5, fig.height=4.5, warning=FALSE}
...
IO.likelihood_block1 <- d.test.IO_block1 %>% 
  ...
  summarise(
    median_likelihood_per_response = median(likelihood_per_response),
    mode_likelihood_per_response = max(likelihood_per_response, na.rm = T)) %>% 
  mutate(cues = ifelse(str_detect(io.type, "VOT_F0.*block1$"), "VOT_F0", "VOT"))


p.likelihood.block1 <- IO.likelihood_block1 %>% 
  mutate(io.label = if(str_detect(io.type, "^(VOT|VOT_F0)\\.no\\.noise.*_block1$")) {"raw"} 
  ...
  mutate(
    cues = ifelse(str_detect(io.type, "VOT_F0.*block1$"), "VOT_F0", "VOT"),
