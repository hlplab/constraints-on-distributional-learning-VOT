...
m.block1 <- lme4::glmer(Response.Voiceless ~ 1 + VOT_gs + (1 + VOT_gs | ParticipantID), data = d.block1, family = "binomial")
summary(m.block1)
...
```{r}
# bootstrapping the CI of the estimates of the block 1 model
set.seed(9824)

# group participants' data and nest df
d.block1.boot <- d.block1 %>% 
  group_by(ParticipantID) %>% 
  nest(data = -ParticipantID) %>% 
  # this samples with replacement, by participants so that each participant will have same number of rows in the final data set
  mutate(samples = map(data, ~ bootstraps(.x, times = 500))) %>% 
  select(ParticipantID, samples) %>% 
  # extract the by participant bootstrapped samples and turn the col into tibble
  unnest(samples) %>% 
  mutate(splits = map(splits, ~ as_tibble(.x))) %>% 
  unnest(splits) %>% 
  # group the samples of each participant by the bootstrap sample id (should have n = number of bootstrap times specified)
  group_by(id) %>% 
  nest(data = -id) %>% 
  mutate(model = map(data, ~ glmer(Response.Voiceless ~ 1 + VOT_gs + (1 + VOT_gs | ParticipantID), data = .x, family = "binomial"))) %>% 
  mutate(Intercept = map_dbl(model, ~ tidy(.x)[[4]][[1]]),
         slope = map_dbl(model, ~ tidy(.x)[[4]][[2]]),
         PSE = descale(-(Intercept/slope), VOT.mean_exp2, VOT.sd_exp2))
