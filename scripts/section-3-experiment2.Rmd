...
VOT.centered.input_block1 <- 
  get_IO_categorization(
    cues = c("VOT_centered"), 
    groups = c("Talker", "gender"), 
    VOTs = seq(0, 85, .5) + (chodroff.mean_VOT - VOT.mean_test),
    F0s = normMel(predict_f0(VOTs)) + (chodroff.mean_f0_Mel - f0.mean_test),
    lapse_rate = lapse_exp2,
    alpha = .1, 
    linewidth = 0.3,
...
p.IOs.VOT.centered.input_block1 <- plot_IO_fit(
  ...
  data.test = d.block1 %>% 
    ...
    distinct(Item.VOT),
  data.percept.PSE = post_sample_block1 %>% 
    mutate(PSE = PSE - (chodroff.mean_VOT - VOT.mean_test),
           .lower = .lower - (chodroff.mean_VOT - VOT.mean_test),
           .upper = .upper - (chodroff.mean_VOT - VOT.mean_test)),
...
d.test.IO_block1 <- 
  ...
  mutate(
    ...
    Item.VOT_centered = Item.VOT + (chodroff.mean_VOT - VOT.mean_test),
