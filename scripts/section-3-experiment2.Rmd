    ...
    log_likelihood_per_response =
      pmap(
        ...
        ~ get_average_log_likelihood_of_perception_data_under_IO(
          observed_inputs = if(str_detect(..3, "VOT\\.centered\\.input_block1$")) {as.list(..1$Item.VOT_centered)} 
          ...
          else if (str_detect(..3, "VOT\\.no\\.noise_block1$")) {as.list(..1$Item.VOT)}
          else if (str_detect(..3, "VOT\\.centered_block1$")) {as.list(..1$Item.VOT)}
          else if (str_detect(..3, "VOT_block1$")) {as.list(..1$Item.VOT)}, 
...
ios <- tibble(
  ...
  Sigma = list(matrix(sqrt(76.719)), matrix(sqrt(168.789))),
  prior = c(.5, .5),
  lapse_rate = c(0, 0),
  lapse_bias = c(.5, .5),
...
io.conditions <- crossing(conditions, ios) %>% 
  cbind(tibble(mu = list(c(VOT = 1.1887), c(VOT = 47.9249), c(VOT = 11.1887), c(VOT = 57.9249), c(VOT = 41.1887), c(VOT = 87.9249)))) %>% 
