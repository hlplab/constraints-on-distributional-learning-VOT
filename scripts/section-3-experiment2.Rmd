...
write_csv(d.test_exposure_for_analysis, "../data/d.test_exposure_for_analysis")
```
...
PSE.fit_mix_block1 <- descale(-(as.numeric(summary(fit_mix_block1)$fixed["mu2_Intercept", 1])) / as.numeric(summary(fit_mix_block1)$fixed["mu2_VOT_gs", 1]), VOT.mean_exp2, VOT.sd_exp2)
...
psychometric_fit_block1 %>% mutate(
  Item.VOT = descale(VOT_gs, VOT.mean_exp2, VOT.sd_exp2)
)
```
...
```{r IO-categorisation-VOT-F0-exposure-centered}
    ...
    log_likelihood_per_response =
      pmap(
        ...
        ~ get_average_log_likelihood_of_perception_data_under_IO(
          observed_inputs = if(str_detect(..3, "VOT\\.centered\\.input_block1$")) {..1$Item.VOT_centered} 
          else if (str_detect(..3, "VOT_F0\\.centered.input_block1$")) {..1$VOT_F0_centered} 
          else if (str_detect(..3, "VOT_F0_block1$")) {..1$VOT_F0} 
          else if (str_detect(..3, "VOT_F0\\.no\\.noise_block1$")) {..1$VOT_F0} 
          else if (str_detect(..3, "VOT_F0\\.centered_block1$")) {..1$VOT_F0}
          else {..1$Item.VOT}, 
...
p.likelihood.univar_block1 <- d.test.IO_block1 %>% 
  ...
  geom_point(aes(y = median_likelihood_per_response), 
             size = 3, 
             position = position_dodge(.2), show.legend = F) +
    ...
    data = d.bootstrap_block1 %>% 
      group_by(io.type, gender) %>% 
            ...
            mode.upper = quantile(mode_sample, probs = .975)),
             mapping = aes(x = io.type, ymin = median.lower, ymax = median.upper, colour = gender),
             position = position_dodge(.2),
             linewidth = 1.5,
             alpha = .6,
             inherit.aes = F,
...
p.likelihood.univar_block1 <- p.likelihood.univar_block1 + remove_axes_titles
```



```{r likelihood-comparison-plots-block1, fig.height=4.5, fig.width=6.5, warning=FALSE, cache=FALSE}
...
p.likelihood.multivar_block1 <- d.test.IO_block1 %>% 
  ...
  geom_text(aes(y = mode_likelihood_per_response, label = "+"), 
             size = 3,
             position = position_dodge(.2),
             show.legend = F) +
  geom_errorbar(
  ...
  #   width = 0,
  #   inherit.aes = F) +
  scale_y_continuous(" ", limits = c(.156, .746), labels = scales::label_number(accuracy = 0.001)) +
  scale_x_discrete("IO Type", 
...
p.title2 <- ggplot(data.frame(l = p.likelihood.multivar_block1$labels$y, x = 1, y = 100) %>% mutate(l = "Median likelihood per response")) +
  geom_text(aes(x, y, label = l), angle = 90, fontface = "bold") +
  theme_void() +
  coord_cartesian(clip = "off")

p.title2 + (p.likelihood.univar_block1 / p.likelihood.multivar_block1) +
  plot_layout(ncol = 2, 
              guides = "collect",
              widths = c(1, 25)) &
  theme(legend.position = "top")
...
temp %>%
  ...
  mutate(PSE = descale(-Intercept / VOT_gs, VOT.mean_exp2, VOT.sd_exp2)) %>%
  # Get HDCI or SE
...
d.PSE <- fit_mix_nested %>% 
  ...
  mutate(PSE = descale(-Intercept/slope, VOT.mean_exp2, VOT.sd_exp2)) %>% 
...
p.PSE <- d.PSE %>% 
  ...
  geom_linerange(aes(ymin = lower, ymax = upper), linewidth = 1.2, position = position_dodge(.3), alpha = .5) +
...
d.params <- temp %>%
  ...
  summarise(Intercept.lower = quantile(Intercept, probs = .025),
            Intercept.median = quantile(Intercept, probs = .5),
            Intercept.upper = quantile(Intercept, probs = .975),
            VOT_gs.lower = quantile(VOT_gs, probs = .025),
            VOT_gs.median = quantile(VOT_gs, probs = .5),
            VOT_gs.upper = quantile(VOT_gs, probs = .975))

p.intercepts <- d.params %>% 
  ggplot(aes(x = Block, y = Intercept.median, colour = Condition, group = Condition)) +
...
p.slopes <- d.params %>% 
  ggplot(aes(x = Block, y = VOT_gs.median, colour = Condition, group = Condition)) +
  geom_point(position = position_dodge(.3), size = 2.5) +
...
```{r make-dataframe-for-logistic-regression-on-predicted-props-by-IOs}
...
ios <- tibble(
  ...
  Sigma = list(matrix(sqrt(80)), matrix(sqrt(278))),
  prior = c(.5, .5),
  lapse_rate = c(0, 0),
  lapse_bias = c(.5, .5),
...
io.conditions <- crossing(conditions, ios) %>% 
  cbind(tibble(mu = list(c(VOT = 5), c(VOT = 50), c(VOT = 15), c(VOT = 60), c(VOT = 45), c(VOT = 90)))) %>% 
...
d.io.categorisation %>%
  group_by(condition) %>% 
  nest() %>% 
  mutate(model = map(data, ~ glm(cbind(n_t, n_d) ~ 1 + VOT, family = binomial, data = .x)),
         intercept = map_dbl(model, ~ tidy(.x)[1,2] %>% pull()),
         slope = map_dbl(model, ~ tidy(.x)[2,2] %>% pull()),  
           PSE = -intercept/slope)
```
...
io.conditions <- crossing(conditions, ios) %>% 
  ...
  nest(io = -c(condition)) %>% 
  crossing(x1) %>% 
  mutate(x1 = map(x1, ~ c(.x))) %>% 
  nest(x = x1)

d.exposure.io.predictions <- io.conditions %>% 
  mutate(categorisation = 
           map2(x, io,
          ~ get_categorization_from_MVG_ideal_observer(x = .x$x1, model = .y, decision_rule = "proportional") %>% 
            mutate(VOT = map(x, ~ .x[1]) %>% unlist()))) %>%
  unnest(cols = categorisation, names_repair = "unique") 


d.exposure.io.predictions %>% 
  group_by(condition) %>% 
  filter(category == "/t/") %>% 
  mutate(response_PSE = abs(response - .5)) %>% 
  group_by(condition) %>% 
  filter(response_PSE == min(response_PSE))
