...
```{r compute-average-likelihood-of-IOs-block1}
...
VOT.centered.input_block1 <- VOT.centered_block1 %>% 
  mutate(io.type = "VOT.centered.input_block1")
  ...
  geom_errorbar(
    data = d.bootstrap_block1 %>% 
      ...
      geom_errorbar(
        ...
        width = 0,
        inherit.aes = F) +
     scale_y_continuous("Median likelihood per response", limits = c(.10736, .10994)) + 
     scale_x_discrete( 
...
p.likelihood.multivar_block1 <- d.test.IO_block1 %>% 
  ...
  geom_errorbar(
    ...
    width = 0,
    inherit.aes = F) +
  scale_y_continuous("Median likelihood per response", limits = c(.168, .746)) +
  scale_x_discrete( 
