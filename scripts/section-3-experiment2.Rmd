...
F0.mean_exp2 <- mean(d.test_exposure_for_analysis$Item.Mel_F0_5ms)
F0.sd_exp2 <- sd(d.test_exposure_for_analysis$Item.Mel_F0_5ms)
...
fit_mix_block1 <- brm(
  ...
  file = "../models/Block1-AEDLVOT-lapse-exp2.rds")
```

(ref:mixture-model-block-1) Plot of by-participant intercepts and slopes estimated from the mixture model

```{r plot-intercepts-slopes-block1-model, fig.width=6, fig.height=3, fig.cap="(ref:mixture-model-block-1)"}
d.ranef <- ranef(fit_mix_block1)
...
```{r IO-categorization-VOT-no-noise-block1, message=F}
...
p.IOs.VOT.no.noise_block1 <- plot_IO_fit(
  ...
  posterior.sample = post_sample_block1,
  PSE_VOT.no.noise_block1
)
```


```{r IO-categorization-VOT-block1, message=FALSE}
VOT_block1 <- 
  get_IO_categorization(
    cues = c("VOT"), 
    groups = c("Talker", "gender"), 
...
PSE_VOT_block1 <- get_PSE_quantiles(data = VOT.no.noise_block1, group = "gender")
...
```{r IO-categorisation-VOT-F0-centered-block1, message=FALSE}
VOT_F0.centered_block1 <- 
  get_IO_categorization(
    cues = c("VOT", "f0_Mel_centered"), 
    groups = c("Talker", "gender"), 
    lapse_rate = lapse_exp2,
    with_noise = T, 
    alpha = .1, 
...
```{r, message=F, fig.cap="Temporary plot to show the expected fit after centering the cues. Note: predicted responses are roughly between .2 and .8"}
...
d.block1_centered <- d.block1 %>% 
      mutate(Item.VOT = Item.VOT + (chodroff.mean_VOT - VOT.mean_exp2)) 

# get new means and SDs of centered items
VOT.mean_exp2_centered <- mean(d.block1_centered$Item.VOT)
VOT.sd_exp2_centered <- sd(d.block1_centered$Item.VOT)   

# Gelman-scale VOT values
d.block1_centered %<>% mutate(VOT_gs = (Item.VOT - VOT.mean_exp2_centered)/(2 * VOT.sd_exp2_centered),
             lapse = lapse_exp2)

# get expected values of the centered VOT under the model
if (file.exists("../models/centered_exposure_block1_exp2.rds")) {
  ...
  epred_centered_block1 <- fit_mix_block1 %>%
    epred_draws(
    newdata = d.block1_centered,
    re_formula = NA,
    ndraws = 2000)
  write_rds(epred_centered_block1, file = "../models/epred_centered_exposure_block1_exp2.rds")
}

# plot and view the expected CI based on the draws 
epred_centered_block1 %>% 
  group_by(VOT_gs) %>% 
  summarise(
  ...
  ggplot(aes(x = descale(VOT_gs, VOT.mean_exp2_centered, VOT.sd_exp2_centered), 
             y = estimate__)) +
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = .1) +
  geom_line(colour = "#333333", linewidth = 1.2) +
  scale_x_continuous("VOT (ms)", limits = c(-55, 75)) +
  scale_y_continuous("Fitted proportion of 't'-responses")
```
...
p.IOs.VOT_F0.centered.input_block1 <- plot_IO_fit(
  ...
  summarise(
    ...
    upper__ = quantile(.epred, probs = .975),
    estimate__ = mean(.epred)) %>% 
    mutate(Item.VOT = descale(VOT_gs, VOT.mean_exp2_centered, VOT.sd_exp2_centered)),
  data.test = d.block1 %>%  
    ungroup() %>% distinct(Item.VOT),
  posterior.sample = post_sample_block1,
  PSE_VOT_F0.centered_block1) 
...
```{r prepare-plots-to-make-IO-plot-grid-block1, fig.width=5, fig.height=8.5}
# make plot grid
...
VOT_F0.centered.input_block1 <- VOT_F0.centered_block1 %>% 
  mutate(io.type = "VOT_F0.centered.input_block1")

# join all 5 IOs into one dataframe
d.IOs_block1 <- 
  rbind(VOT.no.noise_block1, VOT_block1, VOT_F0_block1, VOT_F0.centered_block1, VOT_F0.centered.input_block1) %>% 
...
p.likelihood_block1 <- d.test.IO_block1 %>% 
  group_by(io.type) %>% 
  summarise(
    median_likelihood_per_response = median(likelihood_per_response)) %>%
  mutate(io.type = fct_relevel(io.type, "VOT.no.noise_block1")) %>% 
  ...
  geom_errorbar(
    ...
    width = 0,
    inherit.aes = F) +
  scale_y_continuous("Median likelihood per response (log base 10)", limits = c(.10, .65)) +
   scale_x_discrete( 
     limits = c("VOT.no.noise_block1", "VOT_block1", "VOT_F0_block1", "VOT_F0.centered_block1", "VOT_F0.centered.input_block1"),
     breaks = c("VOT.no.noise_block1", "VOT_block1", "VOT_F0_block1", "VOT_F0.centered_block1", "VOT_F0.centered.input_block1"),
     labels = c(VOT.no.noise_block1 = "VOT", VOT_block1 = "VOT\n(+perceptual noise)", VOT_F0_block1 = "VOT-F0\n", VOT_F0.centered_block1 = "VOT-F0\n (+talker centered)", VOT_F0.centered.input_block1 = "VOT-F0 \n (+exposure-centered)")) +
...
p.likelihood.multivar_block1 <- d.test.IO_block1 %>% 
  ...
  geom_errorbar(
    ...
    mapping = aes(x = io.type, ymin = mode.lower, ymax = mode.upper),
    colour = "#333333",
    size = 1.5,
    alpha = .6,
    width = 0,
    inherit.aes = F) +
  scale_y_continuous("Median log-likelihood \n per response", limits = c(.5, .76)) +
  scale_x_discrete( 
                   limits = c("VOT_F0_block1", "VOT_F0.centered_block1", "VOT_F0.centered.input_block1"),
                   breaks = c("VOT_F0_block1", "VOT_F0.centered_block1", "VOT_F0.centered.input_block1"),
                   labels = c(VOT_F0_block1 = "VOT-F0\n", VOT_F0.centered_block1 = "VOT-F0\n (talker centered)", VOT_F0.centered.input_block1 = "VOT-F0 \n (exposure-centered)")) +
  # theme(axis.title.x = element_blank(),
