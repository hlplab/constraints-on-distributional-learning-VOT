...
```{r plot-catch-trial-performance, fig.height=4, fig.width=6}
p.catch <- d %>%
...
```{r plot-IO-logistic-regression-fit, fig.height=base.height*3+1/2, fig.width=base.width*4}
tokens <- 
  bind_rows(
      d.for_analysis %>%  
        filter(Phase == "test") %>%
...
```{r fitted-categorisation-plots, warning=FALSE, fig.width=base.width*3, fig.height = base.height*2.5, fig.cap="(ref:fitted-categorisation-plots)"}
