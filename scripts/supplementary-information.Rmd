...
get_likelihood_from_grouped_data <- function(
    model,
    data
) {
  data %>%
    group_map(
      .f = ~
        evaluate_model(
          model = model,
          x = .x[["x"]],
          response_category = .x[["Response.Category"]],
...
compare_predicted_and_actual <- function(data) {
  p <- 
    ...
    stat_summary(fun = mean, geom = "line", aes(y = Predicted_posterior), 
                 color = "gray") +
    stat_summary(fun.data = mean_cl_boot, geom = "pointrange", aes(y = Response), 
                 size = 1/4) +
    stat_summary(fun.data = mean_cl_boot, geom = "pointrange", aes(y = Predicted_posterior), 
                 color = "gray", size = 1/4) +
...
df_split_exposure_test <- 
  ...
  select(ExposureGroup, Phase, x, Response.Category, Item.ExpectedResponse)
```
...
if (RESET_MODELS || !file.exists(filename)) {
  best_performing_parameters.bias <-
    optim(
      par = log(mean(range.bias)),
      fn = get_likelihood_from_updated_bias,
      method = "L-BFGS-B",
      lower = log(min(range.bias)),
      upper = log(max(range.bias)),
      control = list(
        fnscale = -1,
        factr = 10^8))
...
m_IA_inferred <- 
  infer_prior_beliefs(
    exposure = df.for_ASP %>% filter(Phase == "exposure"), 
    test = df.for_ASP %>% filter(Phase == "test"), 
...
d.pars %>%
  rename(Predicted_posterior = p_cat) %>%
