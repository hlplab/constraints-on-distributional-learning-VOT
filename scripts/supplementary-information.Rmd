...
```{r plot-labeled-trial-performance}
...
As additional analysis we refitted the same mixed-effects psychometric model with a lapse model that included Block as a predictor to estimate the differences in lapse rates between blocks. The estimated lapse rate (in log-odds) for each block is shown in Table \@ref(tab:hypothesis-table-lapse-by-block). Lapse rates were at their highest in the first block (1.5%) but fell sharply up to the fifth block before rising in the sixth (0.07%) and seventh block. Where estimates were especially low, we note the accompanying wide confidence intervals. Overall, the trend shows some evidence of participants getting familiar with the task but with the very low lapse to begin with, suggests that they were indeed paying attention from the very beginning. 

```{r fit-lapse-by-block}
fit.lapse_by_block <- 
  fit_model(
    data = d.for_analysis,
    phase = "all",
...
make_hyp_table(
  ...
  paste(rep("Block", 9), c(1:9), "< 0"), 
    caption = "This table summarizes estimated lapse rate in each block of the experiment.") %>% 
...
We performed the same hypotheses tests on the exposure block model which was fitted separately from test. Analogous to test, we fitted models in standard and nested formulations to obtain interactions and simple effects using the same weakly regularizing priors. While the standard formulation of the model converged (Tables \@ref(tab:hypothesis-table-exposure-interaction-condition-block) and \@ref(tab:hypothesis-table-exposure-interaction-condition-block-VOT)) the nested formulations returned several divergent transitions during sampling. Divergent transitions give pause for caution in model interpretation (see https://mc-stan.org/misc/warnings.html#runtime-warnings for explanation of warning types). 
A possible solution to eliminating divergent transitions in this model would be to relax the variance of the prior distribution assumed for the slope parameter (VOT_gs in the model formula). Indeed, in earlier fits of the test block data we faced convergence issues when we used the recommended weakly regularising prior (student-t distribution with SD = 2.5; @gelman-prior2008) for the slope parameter. Diagnostic plots revealed a slight bi-modality in the posterior distribution which may have been an impediment to smooth sampling; this was solved by increasing the prior distribution's SD which allow the inclusion of data points that were further from the mean. However, fitting the nested exposure models here with yet more accommodating priors would detract from having an identical set of priors across all models fitted for test and exposure. Given that both the simple effects models returned only a few divergent transitions (3 in the nested block and 9 in the nested condition model) and showed fairly good Rhat values (all 1.0 except for one parameter) and ESS values we opted not to explore further solutions for them at this point.  

Consistent with the trend observed in test blocks, categorization boundaries were clearly separated between the conditions in all exposure blocks and grew progressively larger with more exposure. Evidence for the effect was stronger in the +40 vs. +10 comparison ($24.7 \leq$ BFs $\leq 100.3$) with the difference in PSE reaching its maximum in the final exposure block (`r get_bf(fit_exposure.simple_effects_condition, "mu2_Block6:Condition.Exposure_Shift40vs.Shift10 < 0")`). The difference between +10 and baseline widened in exposure block 2 (`r get_bf(fit_exposure.simple_effects_condition, "mu2_Block4:Condition.Exposure_Shift10vs.Shift0 < 0")`) but narrowed in the final block where evidence in support of the difference also fell substantially (`r get_bf(fit_exposure.simple_effects_condition, "mu2_Block6:Condition.Exposure_Shift10vs.Shift0 < 0")`). The smaller difference in PSE between the +10 and baseline conditions reflected the pattern in test blocks. 
...
### Slopes: between conditions in each exposure block
```{r}
...
```{r hypothesis-table-exposure-simple-effects-block, results='asis', fig.cap=""}
...
table.interaction.exposure.condition_block <-
  ...
  "Block 1 to 2: increased $\\Delta_{PSE}$",
  "Block 2 to 3: increased $\\Delta_{PSE}$",
  "{\\em Block 1 to 3: increased $\\Delta_{PSE}$}"), 2),
...
table.interaction.exposure.condition_block_VOT <-
  ...
  "Block 1 to 2: increased $\\Delta_{slope}$",
  "Block 2 to 3: increased $\\Delta_{slope}$",
  "{\\em Block 1 to 3: increased $\\Delta_{slope}$}"), 2),
