...
In estimating listeners' slope and PSE before they receive exposure to the test talker we used data from the isolated speech database of @chodroff-wilson2017 as that would closely resemble the speech of our test talker. The mean PSE marginalised across all talkers in this database produced a point estimate that equated well with the estimated PSE across all three conditions in pre-exposure test block 1. Given the small number of talkers in the database (13) and the known variability in VOT (@kleinschmidt2019 we divided the database into 5 folds by talker and constructed ideal observers from the data in each fold i.e. data to train each IO never overlapped but each had about a fifth of the data points produced by each talker. An alternative would have been to randomly distribute talkers into the folds. With few talkers in the database this would have implied that each prior PSE estimate would have been based on data from only 3-4 talkers. Since the idea is to simulate an adult L1 US English listener's knowledge the prior should ideally be estimated on as many talkers as possible. The first option strikes the best compromise given the small database we had at hand. 
