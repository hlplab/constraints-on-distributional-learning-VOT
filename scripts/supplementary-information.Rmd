...
# THIS IS CURRENTLY STILL A MESS. REORGANIZE THE IMPORT FROM THE START
...
d.chodroff_wilson %<>%
  ...
  # Select talkers that have both /d/ and /t/ observations with at least 5 observations
  # each
  filter(
    n_min.for_category_and_talker >= 5,
    n_category.for_category_and_talker == 2) %>%
  group_by(speechstyle, Talker, category) %>%
...
d.chodroff_wilson.for_analysis <-
  d.chodroff_wilson %>%
    ...
    filter(f0_Mel > 201, vowel_duration < 450) %>%
    mutate(
      across(
...
# as predictor)
    ...
    prior(student_t(3, 0, 2.5), class = b, resp = VOT),
    prior(student_t(3, 0, 2.5), class = b, resp = f0Mel),
    prior(student_t(3, 0, 2.5), class = b, resp = vowelduration),
    prior(cauchy(0, 2.5), class = sd, resp = VOT),
    prior(cauchy(0, 2.5), class = sd, resp = f0Mel),
    prior(cauchy(0, 2.5), class = sd, resp = vowelduration))
...
bf_VOT <-
  ...
  lf(sigma ~ 1 + category + (1 + category | p | Talker)) +
  lf(alpha ~ 1 + category + (1 + category | p | Talker)) +
  brmsfamily(
    "skew_normal",
    link = "identity",
    link_sigma = "log",
...
bounds <- list()
bounds[["f0_Mel"]] <- range(d.chodroff_wilson.for_analysis$f0_Mel)
bounds[["vowel_duration"]] <- range(d.chodroff_wilson.for_analysis$vowel_duration)
...
saveRDS(DMMM, file = "../models/DMMM-attempt2.rds")
```
