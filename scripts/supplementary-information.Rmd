...
d.chodroff_wilson <-
  get_ChodroffWilson_data(
    ...
    # TO DO: check how much data this excludes and whether the exclusions for 
    # bimodality are perhaps too stringent.
    limits.f0 = c(0, 350),
  ...
  filter(n_category == 2) %>%
  # Sample as many tokens from each unique combination of talker and category
  # as there are available for the talker with the least amount of data.
  group_by(Talker, category) %>%
  sample_n(size = first(n_min)) %>%
  # Only use female talkers (TO DO: revisit whether this makes a difference)
...
m_IO.VOT_f0 <-
  ...
  lift_MVG_to_MVG_ideal_observer(
      ...
      lapse_rate = plogis(fixef(fit_mix_test)[[2]]),
...
sample_proportion_of_participants_per_exposure_test_condition <- .1
...
cat("Changes in normalization achieve maximum likelihood of", best_performing_parameters.normalization$value, "for kappa =", exp(best_performing_parameters.normalization$par))
...
if (RESET_MODELS || !file.exists(filename.parameter)) {
  ...
  d_for_ASP.for_decision_changes <-
    ...
    group_by(ExposureGroup, Phase) %>%
    # Sample proportion of participants from each condition
    filter(Phase == "test" | 
             ParticipantID %in% 
             sample(
    ...
    arrange(ExposureGroup, ParticipantID, Phase, Block, Trial) %>%
    droplevels()
  ...
  d_for_ASP.for_decision_changes.test <- 
    ...
    # beta, and thus cannot affect the optimizaiton of beta.
    filter(Phase == "test", ExposureGroup != "no exposure") %>%
    ungroup() %>%
  ...
  df.bias <- 
    update_decision_bias_by_group( 
      prior = m_IO.VOT_f0,
      beta = best_performing_parameters.bias$par,
      data =  
        d_for_ASP %>%
        group_by(ExposureGroup, ParticipantID, Phase) %>%
        filter(Phase == "exposure", grepl(x = ExposureGroup, pattern = ".*Up to test7")) %>%
...
cat("Changes in decision-making achieve maximum likelihood of", best_performing_parameters.normalization$value, "for beta =", exp(best_performing_parameters.normalization$par))
