...
We analyzed participants' categorisation responses during exposure and test blocks in two separate Bayesian mixed-effects psychometric model [@prins2011]. The psychometric model is an extension of mixed-effects logistic regression that also takes into account attentional lapses. 
...
In addition to replicating our analysis under relaxed assumptions about linearity, this auxliary analysis also sheds light on the causes for the 'zigzag' pattern in the intercept and slope estimates for exposure and test blocks that we reported in the main analysis.
...
compare_predicted_and_actual <- function(data) {
  ...
  p %+% 
    (data %>% mutate(ExposureGroup = gsub("[ABC]A", "", ExposureGroup))) %>%
    plot()
...
d_for_ASP <- 
  d_for_analysis %>%
  mutate(Item.F0_target_Mel = normMel(Item.F0_target_for_generation_script))
  ungroup() %>%
  ...
  rename(VOT.CCuRE = Item.VOT, f0_Mel.CCuRE = Item.F0_target_Mel) %>%
  ...
  mutate(
    ...
    x = map2(VOT.CCuRE, f0_Mel.CCuRE, ~ c(.x, .y)),
  ...
  select(
    ...
    VOT.CCuRE, f0_Mel.CCuRE, Response.Category, Item.ExpectedResponse.Category)
```
...
if (RESET_MODELS || !file.exists(filename.parameter)) {
  ...
  d_for_ASP.for_normalization <-
    ...
    filter(Phase == "test" | ParticipantID == first(ParticipantID))
  ...
  best_performing_parameters.normalization <- exp(best_performing_parameters.normalization$par[1])
  ...
  df.normalized <- 
    update_normalization_and_normalize_test_data(
      kappa = best_performing_parameters.normalization,
      data = d_for_ASP) 
...
if (RESET_MODELS || !file.exists(filename.parameter)) {
  # Reduce to one participant or keep all of them (the latter is much slower)
  d_for_ASP.for_decision_changes <-
    d_for_ASP %>%
    #  filter(ExposureGroup == "Cond Shift0CA_Up to test7") %>%
    ...
    filter(Phase == "test" | grepl(x = ExposureGroup, pattern = ".*Up to test7")) %>%
    group_by(ExposureGroup, Phase) %>%
    # Since it is not computationally feasible to run the optimization over all participants 
    # (unlike for the other two adaptive models, the change model for decision biases is order
    # sensitive), we're sampling 5 participants in each exposure condition
    # 
    # TO DO: change this to sample proportional to amount of data in the condition, rather than
    # a fixed number of subjects.
    filter(Phase == "test" | ParticipantID %in% sample(x = unique(ParticipantID), size = 5)) %>%
    arrange(Phase, Block) %>%
    droplevels()
  
  message("Currently, this function assumes a single order of exposure for each exposure group.", 
          "Consider instead using the participant-specific actual order.")
  
  best_performing_parameters.bias <-
    optim(
      par = log(mean(range.beta)),
  ...
  best_performing_parameters.bias <- exp(best_performing_parameters.bias$par[1])
  saveRDS(best_performing_parameters.bias, filename.parameter)
  
  # Use the optimal parameter to update the model's decision biases and then apply 
  # the model to the test data
  df.bias <- 
    update_decision_bias( 
      prior = m_IO.VOT_f0,
      beta = best_performing_parameters.bias,
      data =  d_for_ASP %>%
        group_by(ExposureGroup, ParticipantID, Phase) %>%
        filter(Phase == "test" | grepl(x = ExposureGroup, pattern = ".*Up to test7")) %>%
        arrange(ParticipantID, Phase, Block) %>%
        droplevels()) %>%
    # Get the posterior from the updated model
    mutate(posterior = 
             future_map(
               posterior, 
    ...
    # (since there is nothing to update, the prior model is not in the df up to this point)
    bind_rows(
      bind_cols(
        prior, 
    ...
    left_join(
      data %>% 
        filter(Phase == "test") %>% 
        # Remove ParticipantID for no exposure test data
        mutate(ParticipantID = as.factor(ifelse(ExposureGroup == "no exposure", NA_character_, as.character(ParticipantID)))) %>%
        group_by(ExposureGroup, ParticipantID) %>%
        select(x, Response.Category) %>%
        nest(data = c(x, Response.Category)),
      by = join_by(ExposureGroup, ParticipantID)) %>%
...
if (RESET_MODELS || !file.exists(filename)) {
  ...
  best_performing_parameters.normalization <- exp(best_performing_parameters.normalization$par[1])
  saveRDS(best_performing_parameters.normalization, filename)
} else {
  best_performing_parameters.normalization <- readRDS(filename)
}
...
m_IA_inferred.fixed_mean <- 
  infer_prior_beliefs(
    ...
    pca.observations = F,
    sample = T, 
    lapse_rate = m_IA.VOT_f0$lapse_rate[[1]],
    m_0 = m_IA.VOT_f0$m,
    file = "../models/IBBU-Tan-Jaeger2022-Experiment2-scaled-f0-uncorrected-m-fixed.RDS",
    warmup = 3000, iter = 4000,
    control = list(adapt_delta = .995, max_treedepth = 15))

model <- m_IA_inferred.fixed_mean
```
...
d.pars %>%
  ...
  mutate(VOT = Item.VOT, ExposureGroup = ifelse(group == "prior", "no exposure", group)) %>%
  left_join(get_test_data_from_stanfit(model)) %>%
  mutate(
    Response = voiced / (voiced + voiceless),
...
## Plotting intercept estimates of exposure and test data
...
d.intercept_estimates <-
  full_join(
    ...
    fit_mix_exposure_nested %>% get_intercepts_and_slopes() %>% 
      mutate(Intercept = Intercept + fixef(fit_mix_exposure)[3] * ((VOT.mean_test - VOT.mean_exposure) / (2 * VOT.sd_exposure)))) %>%
