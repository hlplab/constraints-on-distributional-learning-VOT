...
my_priors_no_RE <- c(prior(student_t(3, 0, 2.5), class = "b", dpar = "mu2")) 
my_priors <-
    c(
...
# recode condition as ordered
# add random effects for by subject slopes in GAMM format?
# recode trial?
m <- brm(
    formula = 
      bf(
        Response.Voiceless ~ 1,
         mu1 ~ 0 + offset(0),
         mu2 ~ 1 + t2(VOT_gs, Trial_gs, by = Condition.Exposure),
        # + 
        #   (1 | ParticipantID) + 
        #   (1 + Condition.Exposure | Item.MinimalPair),
         theta1 ~ 1),
    data = d.GAMM,
    prior = my_priors_no_RE,
    cores = 4,
    chains = 4,
    init = 0,
    iter = 2000,
    ...
    file = paste0("../models/GAMM-test+exposure.rds"))
