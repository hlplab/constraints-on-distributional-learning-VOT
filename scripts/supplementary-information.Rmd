...
talker_m <- lm(f0_5ms_into_vowel ~ 1 + VOT, data = d)
...
d.stimuli %>%
  ...
  geom_abline(intercept = tidy(talker_m)$estimate[1], slope = tidy(talker_m)$estimate[2], alpha = .5) +
...
```{r TEMPORARILY-REIMPORTING-CHODROFF-WILSON-DATA-WITHOUT-EXLCUSION-OR-NORMALIZATION}
# TO DO: this should later be integrated with overall import
d.chodroff_wilson.connected <-
  get_ChodroffWilson_data(
    ...
    categories = c("/d/", "/t/"),
    min.n_per_talker_and_category = 1,
...
d.chodroff_wilson %>%
  ...
  geom_autopoint(aes(shape = speechstyle), alpha = .1) +
  geom_autodensity(aes(linetype = speechstyle), position = "identity", alpha = .5) +
  stat_ellipse(
    aes(
      linetype = speechstyle,
      group = interaction(Talker, category)),
    alpha = .1) +
  stat_ellipse(aes(linetype = speechstyle)) +
  geom_autopoint(
    data = d.for_analysis %>%
      filter(Phase == "test") %>%
  ...
  facet_matrix(vars(c(VOT, f0_Mel, vowel_duration)), layer.lower = c(1, 5:6), layer.diag = 2,
               layer.upper = c(3:6))
```
...
# as predictor)
    ...
    prior(cauchy(0, 2.5), class = sd, resp = VOTgs),
    prior(cauchy(0, 2.5), class = sd, resp = f0Melgs),
    prior(cauchy(0, 2.5), class = sd, resp = voweldurationgs))
...
bf_VOT <-
  bf(VOT_gs ~ 0 + category + (0 + category | p | Talker)) +
  lf(sigma ~ 0 + category + (0 + category | p | Talker)) +
  lf(alpha ~ 0 + category + (0 + category | p | Talker)) +
  brmsfamily(
    "skew_normal",
    link = "identity",
    link_sigma = "log",
...
min(d.chodroff_wilson.for_analysis$f0_Mel_gs)
bf_f0 <-  
  bf(f0_Mel_gs | trunc(lb = -1.4) ~ 0 + category + (0 + category | p | Talker)) +
  lf(sigma  | trunc(lb = -1.4) ~ 0 + category + (0 + category | p | Talker)) +
  brmsfamily(
    "gaussian",
    link = "identity",
    link_sigma = "log")

# Recognizing that vowel duration was censored at upper end
ub_vowel_duration <- max(d.chodroff_wilson.for_analysis$vowel_duration_gs)
bf_vowel_duration <-  
  bf(vowel_duration_gs | trunc(ub = 1.9) ~ 0 + category + (0 + category | p | Talker)) +
  lf(sigma| trunc(ub = 1.9) ~ 0 + category + (0 + category | p | Talker)) +
  lf(alpha | trunc(ub = 1.9) ~ 0 + category + (0 + category | p | Talker)) +
  brmsfamily(
    "skew_normal",
    link = "identity",
    link_sigma = "log",
...
# TO DO: consider using *all* talkers here
DMMM <-
  brm(
...
saveRDS(DMMM, file = "../models/DMMM-attempt2")
```

```{r checking-posterior-predictive}
pp_check(DMMM, resp = "VOTgs")
pp_check(DMMM, resp = "f0Melgs")
pp_check(DMMM, resp = "voweldurationgs")
```
...
<!-- TO DO: context + database intro + explanation -->
...
<!-- TO DO: explain how those were derived -->

# Additional statistical comparisons referred to in the main text
...
## Exposure blocks {sec:SI-analysis-exposure-blocks}
...
### Simple effects of exposure condition within each block
<!-- TO DO -->
...
## Can assumptions of our analyses explain differences between exposure and test blocks? {sec:SI-exposure-test-zigzag}
In the main text, we pointed the 'zigzag' pattern in Figure \@ref(fig:plot-fit-slope-PSE)C: the slopes of listeners' categorization functions were smaller during test than during exposure blocks (BF = XXX). <!-- TO DO: can we run a model that contains exposure and test block and runs: Response ~ Block.Type[effect coded] * VOT + Random effects and everything else identical to your main analysis? No exposure condition. The BF of the interaction term is the evidence we need. --> Here, we present auxiliary analyses that suggest that these differences are not due to biases in our analysis.

### Effects of priors on the estimates for exposure and test blocks
The psychometric mixed-effects analyses followed our previous work [e.g., @hoerberg-jaeger2022; @xie2021] and recommendations in the literature, and used weakly regularizing priors. This raises the question whether the fact that epxosure and test blocks differed in the amount of data they contained (24 unlabelled trials during each exposure block vs. 12 during test) can explain the 'zigzag' pattern evidence in Figure \@ref(fig:plot-fit-slope-PSE)C for slopes and, to a lesser extent, in Figure \@ref(fig:plot-fit-slope-PSE)D for PSEs.

<!-- TO DO: more context needed -->

```{r}
d.intercept_estimates <-
  full_join(
    fit_test_nested %>% get_intercepts_and_slopes(),
    fit_exposure_nested %>% get_intercepts_and_slopes() %>%
      mutate(Intercept = Intercept + fixef(fit_exposure)[3] * ((VOT.mean_test - VOT.mean_exposure) / (2 * VOT.sd_exposure)))) %>%
...
Noticeably absent from the GAMM results is any indication of the 'zigzag' pattern between exposure and test blocks. <!-- TO DO: more here. Three possible reasons: Perhaps the GAMM’s 2D smooth just not sensitive enough. I guess that’s quite possible.
Or it’s the absence of random slopes that masks the wiggliness.
Or the apparent wiggliness in your other analyses has, in fact, something to do with our simplifying assumption that VOT has a linear effect (which the GAMM does not make). -->
...
compare_predicted_and_actual <- function(data) {
  p <-
    data %>%
  ...
  data %>%
    ...
    geom_point() +
    geom_smooth(color = ExposureGroup, alpha = Test) +
    annotate(
      geom = "text",
      label = paste0(
        "R^2 = ",
...
d_for_ASP <-
  ...
  rename(VOT = Item.VOT, f0_Mel = Item.F0_target_for_generation_Mel) %>%
  ...
  select(
    ...
    VOT, f0_Mel, Response.Category, Item.ExpectedResponse.Category)
```
...
#
# HMM, actually since the other algorithms are not order sensitive, they should not depend on how many
# participants
```
...
if (RESET_MODELS || !file.exists(filename.parameter)) {
  ...
  df.normalized %<>%
    ...
    # expected by compare_predicted_and_actual
    mutate(
      VOT = map_dbl(x.x, ~ .x[1]),
...
df.normalized %>% compare_predicted_and_actual()
```
...
```{r, fig.width=7, fig.height=6}
df.bias %>% compare_predicted_and_actual()
```
...
m_IA_inferred.fixed_pars <-
  infer_prior_beliefs(
    ...
    pca.observations = F,
    sample = T,
    lapse_rate = m_IO.VOT_f0$lapse_rate[[1]],
    mu_0 = m_IO.VOT_f0$mu,
    Sigma_0 = m_IO.VOT_f0$Sigma,
    file = "../models/IBBU-Tan-Jaeger2022-Experiment2-scaled-f0-uncorrected-fixed-pars.RDS",
    warmup = 3000, iter = 4000,
    control = list(adapt_delta = .995, max_treedepth = 15))

model <- m_IA_inferred.fixed_pars
```
...
This can be correlated against the actual observed answers XXXXXXXX
