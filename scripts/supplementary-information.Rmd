...
d.io.categorization <- 
  ...
  select(Condition.Exposure, Phase, model_unscaled, Block, intercept_unscaled, slope_unscaled, intercept_scaled, slope_scaled, PSE) %>% 
  mutate(across(c(Phase, Condition.Exposure, Block), factor)) 
```

```{r plot-IO-logistic-regression-estimates}
tokens <- 
  bind_rows(
      d.for_analysis %>%  
        filter(Phase == "test") %>%
...
d.io.categorization %>% 
  ...
  cross_join(
    tibble(
      VOT = seq(-50, 150, 5))) %>% 
  nest(data = VOT) %>% 
  mutate(prediction = map2(.x = data, .y = model_unscaled, ~ predict(.y, newdata = .x))) %>% 
  ...
  facet_grid(Condition.Exposure ~ Phase) +
...
### Slopes: Differences across exposure conditions, across blocks within each condition, and differences in the rate of change between exposure conditions and block
