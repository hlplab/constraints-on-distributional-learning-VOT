...
x <-
  ...
  ungroup() %>%
  filter(Phase == "test") %>%
  distinct(Item.VOT, Item.F0_5ms_Mel) %>%
  arrange(Item.VOT) %>%
  mutate(x = map2(Item.VOT, Item.F0_5ms_Mel, ~ c(.x, .y))) %>%
  pull(x)
...
- Vowel 
  ...
  -**End:** if before a stop, when periodicity becomes irregular or at closure onset; if before a lateral, when formant transition approaches steady state; if before a nasal, when formants show a step-wise shift and when intensity shows a steep decline. 
...
The vowel cut-back ratio was set at 0.33 which translates into a third of a ms vowel reduction for every 1ms of VOT. This ratio followed the estimated vowel duration-VOT trade-off for dip-tip minimal pair tokens reported in @allenMiller. The maximum allowed vowel cut-back was 0.5ms to avoid the short vowel in **dip** becoming too short. 
...
ungroup() %>%
  left_join(d.f0.5ms, by = c("Item.Filename", "Item.VOT")) %>% 
              mutate(Item.F0_5ms_Mel = normMel(Item.f0_5ms))
...
f0.mean_exp1 <- mean(d.test.excluded$Item.F0_5ms_Mel)
f0.sd_exp1 <- sd(d.test.excluded$Item.F0_5ms_Mel)
...
p.ellipse <- d.category_ellipse %>%
  ...
  geom_point(
    data = d_for_analysis %>%
      ...
      summarise(across(c(Item.VOT, Item.F0_5ms_Mel), mean)),
    mapping = aes(x = Item.VOT, y = Item.F0_5ms_Mel, colour = Condition.Exposure, shape = category),
    size = 2,
    shape = c(1, 2, 1, 2, 1, 2),
    alpha = 0.8,
  inherit.aes = F,
    ...
    data = d_for_analysis %>%
      ...
      mutate(
        ...
        Item.F0_5ms_Mel_centered = case_when(
          Condition.Exposure == "Shift0" ~ Item.F0_5ms_Mel + (chodroff.mean_f0_Mel - cond_means$Item.F0_5ms_Mel_mean_test_exposure[1]),
          Condition.Exposure == "Shift10" ~ Item.F0_5ms_Mel + (chodroff.mean_f0_Mel - cond_means$Item.F0_5ms_Mel_mean_test_exposure[2]),
          Condition.Exposure == "Shift40" ~ Item.F0_5ms_Mel + (chodroff.mean_f0_Mel - cond_means$Item.F0_5ms_Mel_mean_test_exposure[3]))) %>%
      group_by(Condition.Exposure, category) %>%
