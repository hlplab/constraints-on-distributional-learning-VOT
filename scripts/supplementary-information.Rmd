...
bf_f0 <-  
  bf(f0_Mel_gs ~ 0 + category + (0 + category | p | Talker)) + 
  lf(sigma ~ 0 + category + (0 + category | p | Talker)) +
  brmsfamily(
    "gaussian", 
    link = "identity", 
...
get_prior(
  formula = bf_VOT + bf_f0 + bf_vowel_duration, 
  d.chodroff_wilson %>% 
    mutate(
      across(
...
# TO DO: first remove pitch-halving instances form f0 data!
DMMM <-
  brm(
