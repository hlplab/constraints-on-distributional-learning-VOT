...
```{r}
get_IBBU_updates <- function(condition, kappa = 72, nu = 72) {
  d.exposure <- 
    d_for_analysis %>% 
    ungroup() %>%
    filter(
  ...
  Sigma_noise <- diag(c(80, 878))
  dimnames(Sigma_noise) <- list(c("VOT", "f0"), c("VOT", "f0"))
  
  update_NIW_ideal_adaptor_incrementally(
    prior = d.chodroff_wilson %>%
      make_NIW_ideal_adaptor_from_data(
        cues = c("VOT", "f0"), kappa = kappa, nu = nu,
    ...
    filter(observation.n %in% c(0, 48, 96, 144)) %>%
    nest(ia = -observation.n) %>%
    mutate(
      Condition.Exposure = .env$condition,
      ...
      cf = map(ia, ~ get_categorization_function_from_NIW_ideal_adaptor(.x, noise_treatment = "no_noise")),
      sf = map2(cf, observation.alpha, ~ stat_function(fun = function(x) as.numeric(.x(map(as.list(x), function(y) c(y, predict_f0(y))), target_category = 2)), alpha = .y, color = colours.condition[condition]))) %>%
...
x <-
  d_for_analysis %>%
  filter(Phase == "test") %>%
  distinct(Item.VOT, Item.F0) %>%
  mutate(x = map2(Item.VOT, Item.F0, ~ c(.x, .y))) %>%
  pull(x)

d.updates <-  
  bind_rows(get_IBBU_updates("Shift0"), get_IBBU_updates("Shift10"), get_IBBU_updates("Shift40")) 

d.updates %<>%
  # Convert updated beliefs into estimated PSE and slope
  get_logistic_parameters_from_model(
    model = ., x = x, model_col = "ia", 
