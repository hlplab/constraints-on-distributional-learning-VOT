...
p.ellipse <- d.category_ellipse %>%
  ...
  geom_point(
    data = d_for_analysis %>%
      ...
      summarise(across(c(Item.VOT, Item.F0_Mel), mean)),
    mapping = aes(x = Item.VOT, y = Item.F0_Mel, colour = Condition.Exposure, shape = category),
    size = 2,
    shape = c(1, 2, 1, 2, 1, 2),
    alpha = 0.8,
  inherit.aes = F,
    ...
    data = d_for_analysis %>%
      ...
      mutate(
        ...
        Item.F0_Mel_centered = case_when(
          Condition.Exposure == "Shift0" ~ Item.F0_Mel + (chodroff.mean_f0_Mel - cond_means$Item.F0_Mel_mean_test_exposure[1]),
          Condition.Exposure == "Shift10" ~ Item.F0_Mel + (chodroff.mean_f0_Mel - cond_means$Item.F0_Mel_mean_test_exposure[2]),
          Condition.Exposure == "Shift40" ~ Item.F0_Mel + (chodroff.mean_f0_Mel - cond_means$Item.F0_Mel_mean_test_exposure[3]))) %>%
      group_by(Condition.Exposure, category) %>%
