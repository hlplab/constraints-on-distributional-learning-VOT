...
p.matrix_connected.norm <- d.chodroff_wilson.norm %>% 
  ...
  theme(legend.position = "top",
        strip.text.x = element_blank())
```
(ref:plot-database-unnormalised) Distribution of L1 US-English talker cues with unnormalized (black) and normalized (gray) test tokens. **Panel A:** Isolated speech of initial /d/ and /t/ syllables by 13 females. **Panel B:** Same as **A** but normalised. **Panel C:** Connected speech of word-initial /d/ and /t/ by 102 females. **Panel D:** Same as **C** but normalised within talker. In  **B** and **D** the mean of talker means was added to each tokens in order to keep tokens in the familiar phonetic space [following @xie2023]

```{r plot-database-unnormalised, fig.height=8.5, fig.width=10}
