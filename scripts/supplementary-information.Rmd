...
p.stimuli.VOT_f0 <- d.stimuli %>%
  ...
  scale_y_continuous("F0", breaks = seq(240, 252, 0.5)) +
...
Labelled trials provided another measure to check participant attention during the experiment. Performance on labelled trials was generally good across all exposure conditions with participants in the +40 condition committing the most errors. This is not unexpected for the +40 condition was the furthest shifted with the most unexpected cue-category mappings. 
```{r labeled-trial-performance-group, message=F}
p.labelled <- d %>%
  ...
  scale_x_continuous("Total errors") +
...
```{r plot-labeled-trial-performance, fig.height=5, fig.width=5 }
...
```{r, fig.height=3.5, fig.width=6, include=FALSE}
d.for_analysis %>%
