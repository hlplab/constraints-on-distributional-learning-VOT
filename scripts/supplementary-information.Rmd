...
get_IBBU_updates <- function(condition, kappa = 72, nu = 72) {
  ...
  update_NIW_ideal_adaptor_incrementally(
    prior = d.chodroff_wilson %>%
      make_NIW_ideal_adaptor_from_data(cues = c("VOT", "f0"), kappa = kappa, nu = nu),
    exposure = d.exposure,
    ...
    mutate(
      observation.n = as.numeric(factor(observation.n, levels = sort(observation.n))),
      observation.n = observation.n / max(observation.n),
      cf = map(ia, ~ get_categorization_function_from_NIW_ideal_adaptor(.x, noise_treatment = "no_noise")),
      sf = map2(cf, observation.n, ~ stat_function(fun = function(x) as.numeric(.x(map(as.list(x), function(y) c(y, predict_f0(y))), target_category = 2)), alpha = .y, color = colours.condition[condition])))
}

d.exposure %>% 
  distinct(VOT) %>%
  ggplot(aes(x = VOT)) +
  get_IBBU_updates("Shift0")$sf +
  get_IBBU_updates("Shift10")$sf +
  get_IBBU_updates("Shift40")$sf +
