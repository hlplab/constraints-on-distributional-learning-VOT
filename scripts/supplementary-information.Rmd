...
d.chodroff_wilson %>% 
  ...
  ggplot(aes(x = VOT, y = f0_Mel, shape = category, colour = category, group = Talker)) +
  geom_point(alpha = .2) +
  geom_label(aes(label = Talker))
```


```{r}
p.matrix_isolated <- d.chodroff_wilson %>% 
  filter(speechstyle == "isolated") %>% 
  group_by(category) %>%
...
p.matrix_connected <- d.chodroff_wilson %>% 
...
```{r}
# normalise cues before handing to plot
...
p.matrix_isolated.norm <- d.chodroff_wilson.norm %>% 
  filter(speechstyle == "isolated") %>% 
  group_by(category) %>%
  ggplot(aes(x = .panel_x, y = .panel_y, color = category, fill = category)) +
  geom_autopoint(alpha = .1) +
...
p.matrix_connected.norm <- d.chodroff_wilson.norm %>% 
  filter(speechstyle == "connected") %>% 
  group_by(category) %>%
  ggplot(aes(x = .panel_x, y = .panel_y, color = category, fill = category)) +
  geom_autopoint(alpha = .1) +
...
For each exposure condition we constructed ideal observers to simulate a learner that fully learned the exposure distribution (i.e. the cumulative distribution by the end of exposure block 3) and who would not suffer from attentional lapses. Such an ideal learner would therefore categorize the stimuli at test based purely on the statistics of exposure input. The optimal categorizations of the test stimuli by each of these IOs were obtained following Bayes' rule under the assumption of equal prior probability for both categories. Following this we multiplied the three IOs' responses at each stimulus to simulate categorization over a large number of trials and fit three separate logistic regressions to the responses with VOT as the sole predictor. We obtained the predicted PSEs from each of the linear models which represent the ideal boundary of each exposure condition. These converged with the boundaries computed from the parameters of the Gaussian distributions that underlie each category within each condition. 

fit a logistic regression to each of the three IO's categorisation responses
...
<!-- TO DO --> WE DID NOT FIT A NESTED BLOCK MODEL FOR EXPOSURE
