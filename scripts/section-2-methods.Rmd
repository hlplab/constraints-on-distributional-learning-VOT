...
d.exposure <- 
  ...
  mutate(labelling = factor(ifelse(Item.Labeled == T, "labeled", "unlabeled")),
         Condition.Exposure = factor(case_when(
           Condition.Exposure == "Shift0" ~ "-18ms",
           Condition.Exposure == "Shift10" ~ "-8ms",
           Condition.Exposure == "Shift40" ~ "+22ms")),
...
d.means <- 
  crossing(
    Condition.Exposure = c("-18ms", "-8ms", "+22ms"),
    category = c("/d/", "/t/")) %>% 
  mutate(
    Block = "Block A", 
    Condition.Exposure = fct_relevel(Condition.Exposure, c("-18ms", "-8ms", "+22ms")),
...
p.histogram_conditions <- 
  ...
  scale_colour_manual(
    "Stimulus type",
    values = c(
      "-18ms /d/ labeled" = "#800000", 
      "-18ms /d/ unlabeled" = "#ff9999",
      "-18ms /t/ labeled" = "#cc0000", 
      "-18ms /t/ unlabeled" = "#ffe6e6",
      "-8ms /d/ labeled" = "#0a751c",
      "-8ms /d/ unlabeled" = "#b9f9c3",
      "-8ms /t/ labeled" = "#12D432",
      "-8ms /t/ unlabeled" = "#e8fdeb",
      "+22ms /d/ labeled" = "#02427e", 
      "+22ms /d/ unlabeled" = "#b4dafe",
      "+22ms /t/ labeled" = "#0481F3", 
      "+22ms /t/ unlabeled" = "#e6f3ff"),
