...
Half of the /d/ and half of the /t/ trials were labeled, the other half was unlabeled [paralleling one of the conditions in @kleinschmidt2015cogsci]. Unlabeled trials were identical to test trials except that the distribution of VOTs across those trials was bimodal (rather than uniform), and determined by the exposure condition.^[Since previous studies largely lacked test blocks, these studies estimated changes in participants' categorisation responses by analyzing responses on unlabeled exposure trials [e.g., @clayards2008; @kleinschmidt-jaeger2016; @theodore-monto2019]. This approach compares responses across different values of acoustic-phonetic cues (since the exposure inputs differed by exposure condition), so that assumptions baked into the analysis approach (e.g., linearity along the acoustic-phonetic continuum) can potentially bias the results. Here we avoid this issue by holding test stimuli constant.] Labeled trials instead presented two response options with identical stop onsets (e.g., *din* and *dill*). This effectively labeled the input as belonging to the intended category (e.g., /d/). 
...
p.histogram_conditions <- 
  ...
  # stat_function(fun = function(x) 24 * 5 * dnorm(x, 5, sqrt(var_d)),
  #               color = "black", linewidth = .6, alpha = .7, linetype = 2) +
  # stat_function(
  #   fun = function(x) 24 * 5 * dnorm(x, 50, sqrt(var_t)),
