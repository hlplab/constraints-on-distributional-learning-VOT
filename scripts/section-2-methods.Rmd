...
p.histogram_conditions <- 
  ...
  filter(image_selection == "forward" & list_LSQ_variant == "A") %>%
    ...
    block == 6 ~ "Block C")) %>% bind_rows(
      d.exposure_trials %>% na.omit() %>% filter(image_selection == "forward" & list_LSQ_variant == "A") %>% mutate(block = "Block D")) %>%
  ...
  facet_grid(
    condition ~ block, 
    labeller = labeller(block = c("Block A" = "Block A", "Block B" = "Block B", "Block C" = "Block C", "Block D" = "all")), # facet not respecting factor order
    scales = "free_y") +
