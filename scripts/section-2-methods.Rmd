...
(-5, 5, 15, 25, 30, 35, 40, 45, 50, 55, 65, 70 msec). A uniform distribution over VOTs was chosen to maximize the statistical power to determine participants' categorisation function. The assignment of VOTs to minimal pair continua was counter-balanced within and across test blocks, so that each minimal pair continuum appear equally often within each test block (four times), and each minimal pair appear with each VOT equally often (twice) across all six test blocks (and no more than once per test block). <!-- TO DO: correct? -->
...
Both the placement of the response options (/d/ on the left vs. right) and the assignment of VOTs to minimal pair continua was counter-balanced across participants, using 2 x 3 Latin-square designed lists. <!-- TO DO: correct? --> Trial order was randomized within each block and participant. 
...
The distribution of VOTs across the 48 /d/-/t/ trials depended on the exposure condition. Specifically, we first created a *baseline* condition. Although not critical to the purpose of the experiment, we aimed for the VOT distribution in this condition to closely resemble participants' prior expectations for a `typical' female talker of L1-US English (for details, see SI, \@ref(XXX)). The mean and standard deviations for /d/ along VOT were set XXX msecs and XXX msecs, respectively. The mean and standard deviations for /t/ were set XXX msecs and XXX msecs, respectively. 

To create more realistic VOT distributions, we *sampled* from the intended VOT distribution (top row of Figure \@ref(fig:exp2-design-distribution)). This creates distributions that more closely resemble the type of distributional input listeners experience in everyday speech perception, deviating from previous work, which exposed listeners to highly unnatural fully symmetric samples [@clayards2008; @kleinschmidt-jaeger2016; @kleinschmidt2020]. We created different two random samples for each of the three exposure blocks. Both the random seed and the order of exposure blocks was counter-balanced across participants using 2 (seed) x 3 (block order) Latin-squared designed exposure lists. <!-- TO DO: correct? -->

Half of the /d/ and half of the /t/ trials were labeled, the other half was unlabeled [paralleling one of the conditions in @kleinschmidt2015cogsci]. Unlabeled trials were identical to test trials except that they distribution of VOTs across those trials was bimodal (rather than uniform), and determined by the exposure condition. Labeled trials instead presented two response options with identical stop onsets (e.g., *din* and *dill*). This effectively labeled the input as belonging to the intended category (e.g., /d/). 
...
(ref:exp2-design-distribution) Histogram of VOTs across the 48 trials of all three exposure blocks by exposure condition. Shown is one of two parameterizations of the VOT samples used across experimental lists. The order of blocks was counter-balanced across participants. <!-- TO DO: could you split this by block, so that readers can understand how it varied by block? call it exposure block A, B, C -->

```{r exp2-design-distribution, fig.height=2.8, fig.width=6, fig.cap="(ref:exp2-design-distribution)",warning=FALSE, message=FALSE}
d.exposure_trials <- 
  read_csv("../data/exposure_block_tokens.csv", show_col_types = F) %>% 
  mutate(
    condition = case_when(condition == "Shift0" ~ "+0ms",
...
The order of trials was randomized within each block and participant, with the constraint that no more than two catch trials would occur in a row. <!-- TO DO: more on counter-balancing here, paralleling what I wrote for tets --> Participants were randomly assigned to one of the 3 (exposure condition) x 2 (random seed) x 3 (block order) exposure lists. 
