...
```{r design-distribution, fig.height=base.height*3+1/2, fig.width=base.width*4, fig.cap="(ref:exp2-design-distribution)", warning=FALSE, message=FALSE}
...
# GEOM RUG IS NOT SHOWING IN PLOT. HAVEN'T FIGURED OUT WHY  
p.histogram_conditions <- 
  ...
  geom_histogram(
    aes(x = VOT, 
        fill = paste(Condition.Exposure, category, labelling), 
        color = paste(Condition.Exposure, category, labelling),
        linetype = labelling), 
    alpha = .8) +
  geom_text(
    data = d.means,
