...
source("functions.R")
```
To assess how exposure incrementally and cumulatively affects listeners' perception, we employed the repeated exposure-test design shown in \@ref(fig:exp2-design-figure). The use of test blocks that repeated same stimuli across blocks and exposure conditions deviates from previous work [@clayards2008; @kleinschmidt-jaeger2016; @kleinschmidt2020]. This design feature allows us to assess how increasing exposure affects listeners' perception without making strong assumptions about the nature of these changes (e.g., linear changes across trials).
...
### Materials 
We recorded XXX tokens each of four minimal word pairs ("dill"/"till", "dim"/"tim", "din"/"tin", and "dip"/"tip") from a 23-year-old, female L1-US English talker from New Hampshire, judged to have a "general American" accent. These recordings were used to create four natural-sounding minimal pair VOT continua using a script [@winn2020manipulation] in Praat [@praat]. A norming experiment (N = XXX participants) reported in the SI (@\ref(XXX)) was used to select the three minimal pairs that elicited the most similar categorization responses (dill-till, din-tin, and dip-tip).  
...
The conditions were created by first generating a baseline distribution and then shifting the baseline by +10ms and by +40ms towards the right of the VOT continuum to create the remaining two conditions. 
...
(ref:exp2-design-distribution) Histogram of all 144 exposure tokens presented over three blocks by exposure condition. 
