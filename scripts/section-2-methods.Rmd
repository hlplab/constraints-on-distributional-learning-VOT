...
p.placement_of_exposure_means_relative_to_prior <-
  ...
  facet_matrix(
    ...
    labeller = labeller(
      .rows = c(VOT = "VOT", f0_Mel = "f0", vowel_duration = "V duration"),
      .cols = c(VOT = "VOT", f0_Mel = "f0", vowel_duration = "V duration"))) +
...
d.condition.exposure.IO %>%
  ...
  scale_x_continuous(limits = c(-10, 95), breaks = c(25, 35, 45, 65)) +
  guides(colour = "none") +
...
ggplot() +
 ...
 geom_segment(
  ...
  annotate(
       ...
       x = c(rep(1.2, 3)),
       y = c(25, 35, 65),
       label = #c(expression("" %~~% "-20 ms"), expression("" %~~% "-10 ms"), expression("" %~~% "+20 ms")),
# this is the only way I can get Rmd to render the approximate symbol. Tried with the UTF code but it doesn't render
d.prior.io %>%
         summarise(PSE_median = floor(median(PSE))) %>%
         uncount(3) %>%
       ...
       fontface = "bold",
       size = 2.5) +
  scale_y_continuous("PSE (ms VOT)", limits = c(18, 68), breaks = c(25, 35, 45, 65)) +
