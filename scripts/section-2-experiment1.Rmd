...
d.chodroff_wilson.selected %>% 
  ungroup() %>% 
  summarise(across(c("VOT", "f0_Mel", "VOT_centered", "f0_Mel_centered"), mean)) %>% 
  pull(f0_Mel_centered)
```
...
d.test.excluded %>% 
  ungroup() %>% 
    summarise(across(c("Item.VOT", "Item.f0_5ms", "Item.Mel_f0_5ms"), mean))
```
...
```{r}
# make plot grid
legend1 <- get_legend(p.likelihoods.exposure.centered)
legend2 <- get_legend(p.IOs.VOT_F0.centered.input)
...
```{r, comparative-IO-plot, warning=FALSE, fig.width=6, fig.height=8, fig.cap="(ref:comparative-IO-plot)"}
