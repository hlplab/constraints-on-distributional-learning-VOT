...
centered_exposure %>% 
  ...
  ggplot(aes(x = descale(sVOT, VOT.mean_exp1, VOT.sd_exp1) - (chodroff.mean_VOT - VOT.mean_exp1), #ensures x-value for the y-value is same as bef
             y = estimate__, linetype = "solid")) +
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = .05, fill = "#333333") +
  geom_line(colour = "#333333",
            linewidth = 1.2,
            alpha = .5) +
  geom_ribbon(
    data = psychometric_fit_exp1 %>% 
  group_by(sVOT) %>% 
  summarise(estimate__ = mean(estimate__),
         lower__ = mean(lower__),
         upper__ = mean(upper__)) %>% 
  ...
  scale_y_continuous("Fitted proportion of 't'-responses") +
  scale_linetype_manual("Predictions", values = c("solid", "dashed"), labels = c("dashed"= "uncentered", "solid" = "centered"))
```
...
p.likelihoods.exposure.centered <- plot_talker_MVGs(VOT_F0.centered, cues = c("VOT_centered", "f0_Mel_centered"), centered = T)
```
...
# make VOT_F0_centered data set
VOT_F0.centered.input <- VOT_F0.centered %>% 
  mutate(io.type = "VOT_F0.centered.input")

VOT.centered.input <- VOT.centered %>% 
  mutate(io.type = "VOT.centered.input")
...
p.likelihood <- d.test.IO %>% 
  ...
  mutate(io.type = fct_relevel(io.type, c("VOT.no.noise", "VOT", "VOT.centered", "VOT.centered.input", "VOT_F0.no.noise")),
  ...
  geom_errorbar(
    ...
    position = position_dodge(.2),
    inherit.aes = F) +
  scale_y_continuous("Median likelihood per response", limits = c(0.07385, 0.07392)) +
   scale_x_discrete( 
...
```{r IO-multivar-likelihood-plot, fig.width=5.5, fig.height=3.5, warning=FALSE, fig.cap="(ref:comparing-likelihoods-of-perception-data-under-each-bivariate-IO)"}
