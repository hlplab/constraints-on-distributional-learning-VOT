...
```{r comparative-IO-plot, warning=FALSE, fig.width=6, fig.height=4, fig.cap="(ref:comparative-IO-plot)"}
p1 <- 
  p.IOs.VOT.no.noise + 
...
IO_grid1 <- add_sub(IO_grid1, "VOT (ms)", x = .55, size = 10, fontface = "bold", vjust = -1)
IO_grid1 <- add_sub(IO_grid1, "Proportion of 't'-responses", x = -0.03, y = 2.5, size = 10, fontface = "bold", angle = 90)
IO_grid2 <- plot_grid(legend, IO_grid1, align = "h", ncol = 1, rel_heights = c(.08, 1))
IO_grid2
```


```{r temporary-chunk-poster-plotting, warning=FALSE, fig.height=2, fig.width=8}
p1 <- 
  p.IOs.VOT.no.noise + 
  theme(legend.position = "none", 
        axis.title.x = element_blank()) 
...
IO_grid2 <- add_sub(IO_grid2, "VOT (ms)", x = .55, size = 10, fontface = "bold", vjust = 0)
...
ggsave("../figures/IO_plot_exp1.png", plot = p, width = 25, height = 5, units = "cm", dpi = 300)
```
...
d.test.IO %>% 
  group_by(io.type) %>% 
  summarise(
    # Here you could use bootstrap to get the lower and upper CI within each group
    mean_log_likelihood_per_response = mean(likelihood_per_response) 
  ) %>% 
  mutate(mean_accuracy_per_response = exp(mean_log_likelihood_per_response))

# use bootstrap samples by io type to find the CI of the mean estimates
d.bootstrap <- d.test.IO %>% 
  ...
  mutate(splits = map(splits, ~ as_tibble(.x)),
         mean_sample = map_dbl(splits, ~ mean(.x$likelihood_per_response)))
```
...
# plot mean likelihood per response with bootstrapped 95% CI
d.test.IO %>% 
  group_by(io.type) %>% 
  summarise(
    mean_likelihood_per_response = mean(likelihood_per_response)) %>%
  mutate(io_cues = ifelse(str_detect(io.type, "VOT_F0"), "VOT-F0", "VOT")) %>% 
  filter(io_cues == "VOT-F0") %>% 
  ggplot(aes(x = io.type, 
             y = mean_likelihood_per_response, colour = io.type)) +
  geom_point(size = 3) +
  geom_errorbar(
    data = d.bootstrap %>% 
      group_by(io.type) %>% 
  ...
  filter(io_cues == "VOT-F0") %>% 
      summarise(
        mean.lower = quantile(mean_sample, probs = .025),
        mean.upper = quantile(mean_sample, probs = .975)
      ),
    mapping = 
      aes(x = io.type, ymin = mean.lower, ymax = mean.upper),
    colour = c("#FCD900", "#E8630A", "#B22727"),
    size = 1.5,
    alpha = .6,
  width = 0,
    inherit.aes = F) +
  scale_colour_manual("Model", values = c("#FCD900", "#E8630A", "#B22727"), 
                      labels = c("VOT-F0", "VOT-F0-centered", "VOT-F0-input-centered")) +
...
d.test.IO %>% 
  ...
  filter(io_cues == "VOT") %>% 
    ...
    mapping = 
      aes(x = io.type, ymin = mean.lower, ymax = mean.upper),
    colour = c("#FCD900", "#E8630A"),
    size = 1.5,
    alpha = .6,
  width = 0,
    inherit.aes = F) +
  scale_colour_manual("Model", values = c("#FCD900", "#E8630A"), 
                      labels = c("VOT-no-noise", "VOT-noise")) +
  theme(axis.title.x = element_blank(),
        legend.position = "top")
```
