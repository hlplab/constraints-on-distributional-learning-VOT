...
```{r IO-multivar-likelihood-plot, fig.width=6.5, fig.height=4.5, warning=FALSE, fig.cap="(ref:comparing-likelihoods-of-perception-data-under-each-bivariate-IO)"}
p.likelihood.multivar <- d.test.IO %>% 
  ...
  #   position = position_dodge(.2),
  #   inherit.aes = F) +
  scale_y_continuous(" ", limits = c(.05, .67)) +
  scale_x_discrete(expand = expansion(mult = c(0.09, 0.09)), 
                   ...
                   labels = c(VOT_F0.no.noise = "VOT-F0", VOT_F0 = "VOT-F0\n(+perceptual noise)", VOT_F0.centered = "VOT-F0\n (+talker-centered)", VOT_F0.centered.input = "VOT-F0 \n (+exposure-centered)")) +
  scale_colour_manual("Gender", values = colours.sex, labels = c("Female", "Male")) +
  theme(axis.title.x = element_blank(),
        legend.position = "none") +
  guides(axis.text.x = element_text(size = 8)) +
  coord_trans(y = "log10") 


p.title <- ggplot(data.frame(l = p.likelihood.multivar$labels$y, x = 1, y = 100) %>% mutate(l = "Median likelihood per response")) +
  geom_text(aes(x, y, label = l), angle = 90, fontface = "bold") +
  theme_void() +
  coord_cartesian(clip = "off")


p.title + (p.likelihood / p.likelihood.multivar) +
  plot_layout(ncol = 2, 
              guides = "collect",
              widths = c(1, 25)) &
  theme(legend.position = "top")
