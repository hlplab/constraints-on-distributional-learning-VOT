...
```{r brms-model-with-f0, fig.width=3.5, fig.cap="temporary plot of effects of VOT on categorisation from model that includes F0. Red dashed line is model with F0."}
# run an alternative model with f0 as a predictor
# get f0 mean and sd for scaling
F0_Mel.mean_norm <- mean(d.test.excluded$Item.Mel_f0_5ms)
F0_Mel.sd_norm <- sd(d.test.excluded$Item.Mel_f0_5ms)
F0.mean_norm <- mean(d.test.excluded$Item.f0_5ms)
d.test.excluded %<>% 
  mutate(sF0 = (Item.Mel_f0_5ms - F0_Mel.mean_norm)/(2 * F0_Mel.sd_norm))
...
fit_mix_f0 <- brm(
  ...
  family = mixture(bernoulli("logit"), bernoulli("logit")),
  control = list(adapt_delta = .99),
  file = "../models/Exp-NORM-lapsing-bias-f0.rds")
...
psychometric_fit_data %>% 
    ...
    plot = F)[[1]], 
    mapping = aes(
      x = descale(sVOT, VOT.mean_norm, VOT.sd_norm),
      ymin = lower__, 
      ymax = upper__),
    alpha = 0.05,
    inherit.aes = F) +
  ...
  scale_y_continuous("Fitted proportion of 't' responses") +
  scale_linetype_manual("Model", values = c("solid" = 1, "dashed" = 2), labels = c("fit without F0", "fit with F0"))
```
...
```{r get-expected-predictions-exposure-centered-VOT, warning=FALSE, fig.width=3.7, fig.height=2.5, fig.cap="temporary plot of expected categorization function under the model with F0 after centering exposure cues."}
...
chodroff.mean_f0 <- chodroff.means %>% pull(f0)
...
# plot the expected categorization function 
centered_exposure %<>% 
  group_by(sVOT) %>% 
  summarise(
...
centered_exposure %>% 
  ggplot(aes(x = descale(sVOT, chodroff.mean_VOT, VOT.sd_norm), 
             y = estimate__)) +
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = .1) +
  geom_line(colour = "#333333", linewidth = 1.2) +
  scale_x_continuous("VOT (ms)", limits = c(-15, 75)) +
  scale_y_continuous("Fitted proportion of 't'-responses")
```


```{r plot-IO-VOT-F0-exposure-centered}
p.IOs.VOT_F0.centered.input <- plot_IO_fit(
  data.production = VOT_F0.centered, 
  data.perception = centered_exposure %>% 
    mutate(Item.VOT = descale(sVOT, chodroff.mean_VOT, VOT.sd_norm)),
  data.test = d.test.excluded %>% 
    ungroup() %>% distinct(Item.VOT),
  posterior.sample = post_sample_norm,
  PSE_VOT_F0.centered) 
...
d.test.IO <- 
  ...
  mutate(
    ...
    Item.Mel_F0_centered = Item.Mel_f0_5ms - (chodroff.mean_f0 - F0.mean_norm), 
...
```{r IO-likelihood-plot, fig.width=6, fig.height=3, warning=FALSE}
# plot median likelihood per response with bootstrapped 95% CI
p.likelihood <- d.test.IO %>% 
  group_by(io.type) %>% 
  summarise(
    median_likelihood_per_response = median(likelihood_per_response)) %>%
  mutate(io.type = fct_relevel(io.type, "VOT.no.noise")) %>% 
...
```{r IO-likelihood-plot-enlarged, fig.width=4, fig.height=3, warning=FALSE}
...
(ref:IO-likelihood-plot) Median likelihood estimates of the human behavioural data under the assumptions of each IO model. Inset: Triangles indicate the mode, error bars represent the 95% quantile intervals from 1000 bootstrap samples of by-talker estimates of each IO type.

```{r combined-IO-likelihood-plots, fig.width=8, fig.height=6.5, fig.cap="(ref:IO-likelihood-plot)", warning=FALSE}
