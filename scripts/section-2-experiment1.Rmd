...
d.temp <- d.test.excluded %>% 
  ...
  crossing(d.IOs) %>% 
  mutate(x = 
           map2(d.perception, io.type, ~
                  ifelse(str_detect(.y, "VOT_F0"), tibble(.x$VOT_F0), tibble(.x$Item.VOT)))) %>% 
  nest(x = x) %>% 
  mutate(accuracy = 
           pmap(list(x, d.perception, io), ~ 
                  get_average_accuracy_of_IO(observations = ..1$x, responses = ..2$Response.category, model = ..3)))
```
