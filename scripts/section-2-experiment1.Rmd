...
```{r IO-likelihood-plot, fig.width=4, fig.height=4, warning=FALSE}
...
p.likelihood <- d.test.IO %>% 
  ...
  geom_errorbar(
    ...
    position = position_dodge(.2),
    inherit.aes = F) +
  scale_y_continuous("Median likelihood per response", limits = c(.081, .0813)) +
   scale_x_discrete( 
     limits = c("VOT.no.noise", "VOT"),
     breaks = c("VOT.no.noise", "VOT"),
...
```{r IO-likelihood-plot-enlarged, fig.width=5, fig.height=3, warning=FALSE}
