...
io.gender.univar.no.noise <- get_IO_categorization(cues = c("VOT"), with_noise = FALSE, groups = "gender", alpha = 1, size = 1) %>% 
...
io.gender.multivar <- get_IO_categorization(cues = c("VOT", "f0_Mel"), groups = "gender", alpha = 1, size = 1) %>% 
...
d.IOs <- rbind(io.talker.univar.no.noise, io.talker.univar, io.talker.multivar, io.talker.centered) %>% 
...
d.temp <- d.test.excluded %>% 
  ...
  mutate(accuracy = 
           pmap(list(d.perception, io, io.type), ~
                  get_average_accuracy_of_IO(observations = ifelse(str_detect(..3, "io.talker.univar"), ..1$Item.VOT, ..1$VOT_F0), 
