...
```{r brms-model-with-f0}
# run an alternative model with f0 as a predictor
# get f0 mean and sd for scaling
F0.mean_norm <- mean(d.test.excluded$Item.Mel_f0_5ms)
F0.sd_norm <- sd(d.test.excluded$Item.Mel_f0_5ms)
d.test.excluded %<>% 
  mutate(sF0 = (Item.Mel_f0_5ms - F0.mean_norm)/(2 * F0.sd_norm))
...
psychometric_fit_data %>% 
  ...
  scale_linetype_manual("Model", 
                        values = c(1, 2),
                        labels = c("fit without F0", "fit with F0"))
```




```{r plot-IO-VOT-no-noise, fig.width=3.7, fig.height=2.5}
...
```{r plot-IO-VOT-with-noise, fig.width=3.7, fig.height=2.5}
...
```{r plot-IO-VOT-F0, warning=FALSE, fig.width=3.7, fig.height=2.5}
...
```{r get-expected-predictions-exposure-centered-VOT, warning=FALSE, fig.width=3.7, fig.height=2.5}
...
d.test.IO <- 
  ...
  mutate(
    ...
    Item.Mel_F0_centered = Item.Mel_f0_5ms - (chodroff.mean_f0 - F0.mean_norm), # should this calculation be
