...
In addition to the critical minimal pairs we also recorded three words that did not did not contain any stop consonant sounds ("flare", "share", and "rare"). These word recordings were used as catch trials. Stimuli intensity were standardised at 70 dB sound pressure level. 
...
The trials were presented randomly and the order of the written word forms were counter-balanced across participants. Participants were given 192 target trials along a 24-step VOT continuum to categorise. VOT tokens in the lower and upper ends were distributed over larger increments because stimuli in those ranges were expected to elicit floor and ceiling effects, respectively. Each word pair was played twice at each VOT step, constituting a uniform distribution. In addition to the critical trials, 12 catch trials were inserted randomly throughout the experiment. These trials served as a check on participant attention throughout the experiment. Participants were given the opportunity to take breaks after every 60 trials. Participants spent an average of 12 minutes (SD = 4.8) on the trials after which they answered a short survey about the experiment. 
...
# mark catch trials rows and check correct
 d.test %<>% mutate(Is.CatchTrial = ifelse(Item.ExpectedResponse %in% c("flare", "rare", "share"), TRUE, FALSE),
...
excluded.data.due.to.catch <- d.test %>% 
  group_by(ParticipantID) %>%
...
d.test %<>%
  ...
  mutate(Response.log_RT = log10(ifelse(Response.RT <= 0, NA_real_, Response.RT)),
         ...
         Excluded.participant.due.to.mean.RT = ifelse(abs(scale(Response.log_RT.mean)) > 3, TRUE, FALSE)) # scale by mean of participant means

excluded.participants.due.to.mean.RT <- d.test %>% 
  filter(Excluded.participant.due.to.mean.RT == TRUE)

d.test %<>% 
  filter(Excluded.due.to.CatchTrial == FALSE |
           Excluded.participant.due.to.mean.RT == FALSE) %>% 
  mutate(Excluded.participant.due.to.RT = ifelse(abs(Response.log_RT.scaled) > 3, TRUE, FALSE))
  
# get the rows removed due to RT
excluded.data.due.to.RT <- d.test %>% 
  filter(Excluded.participant.due.to.mean.RT == TRUE | 
...
We fit the model using the package \texttt{brms} [@R-brms_a] in R [@R; @RStudio]. Following our previous work [@horberg2021rational; @xie2021cross], we used weakly regularizing priors to facilitate model convergence. For fixed effect parameters, we standardized continuous predictors (VOT) by dividing through twice their standard deviation [@gelman2008standardize], and used Student priors centered around zero with a scale of 2.5 units [following @gelman2008weakly] and 3 degrees of freedom. For random effect standard deviations, we used a Cauchy prior with location 0 and scale 2, and for random effect correlations, we used an uninformative LKJ-Correlation prior with its only parameter set to 1, describing a uniform prior over correlation matrices [@Lewandowski2009]. Four chains with 2000 warm-up samples and 2000 posterior samples each were fit. No divergent transitions after warm-up were observed, and all $\hat{R}$ were close to 1.

```{r}
# load the psychometric model with interaction
fit_mix <- brm(
  bf(
...
psychometric_fit_data <- 
  conditional_effects(
    fit_mix, 
    effects = "sVOT", 
    #re_formula = NULL, # by default conditional_effects excludes random effects when calculating predictions
    plot = F)[[1]]

int_conditions <- list(sTrial = quantile(sort(unique((d.test.excluded$Trial - Trial.mean)) / (2 * Trial.sd))))
...
MinPair_plot <- cat_minimalpair %>% 
  ...
  stat_lineribbon(alpha = .6) +
  scale_x_continuous("VOT (ms)") +
  scale_y_continuous("Fitted proportion of 't'-responses") +
  scale_color_brewer("Minimal Pair", palette = "Blues", labels = c("dill/till", "dim/tim", "din/tin", "dip/tip")) +
...
p.vot_trial <- p.psychometric.vot %+%
  (filter(psychometric_fit_interaction, effect2__ == .5)) +
  geom_ribbon(
    aes(ymin = lower__, 
        ymax = upper__), alpha = .08) +
  ...
  theme(axis.title.y = element_blank(),
        ...
        axis.ticks.y = element_blank())
```
(ref:fitted-categorisation-plots) Fitted categorisation functions to listeners' responses marginalizing over trial effects as well as all random effects (left) and combined effects of VOT and trial (middle), marginalizing over all random effects. Vertical point ranges represent the mean proportion of t-responses at respective VOTs and vertical bars denote the 95% bootstrapped confidence interval. Black error bar (bottom of left panel) denotes the 95% quantile interval of the points of subjective equality (PSE), derived from 8000 sample draws from the posterior distribution of estimated population parameters. Rightmost plot shows the predicted categorisation functionsfor all four minimal pair items.   

```{r fitted-categorisation-plots, warning=FALSE, fig.width=6.8, fig.height = 2.8, fig.cap="(ref:fitted-categorisation-plots)"}
legend <- get_legend(MinPair_plot + theme(legend.position = "top"))
...
```{r}
fit_mix %>% 
  gather_draws(r_ParticipantID__theta1[ParticpantID, term], b_theta1_Intercept)
```


\newpage

```{r bivariate-model-brms, echo=FALSE}
# d.chodroff_wilson <-
...
# 
# contrasts(d.chodroff_wilson.selected$category) = cbind("/t/ vs. /d/" = c(-.5, .5))
# contrasts(d.chodroff_wilson.selected$gender) = cbind("female vs. male" = c(-.5, .5))
# 
# 
# bf_VOT <- bf(
#   VOT_centered ~ 1 + category * gender + (1 + category | tm | Talker) + (1 | wm | Word),
#   sigma ~ 1 + category * gender + (1 + category | ts | Talker) + (1 | ws | Word))
# 
# bf_f0_Mel <- bf(
#   f0_Mel_centered ~ 1 + category * gender + (1 + category | tm | Talker) + (1 | wm | Word),
#   sigma ~ 1 + category * gender + (1 + category | ts | Talker) + (1 | ws | Word))
#   
# cues <- c("VOTcentered", "f0Melcentered")
# randoms <- c("Talker", "Word")
# 
# my.priors.bivariate = c(
#   # weakly regularizing prior for fixed effect coefficients for mu.
#   set_prior("student_t(3, 0, 2.5)", class = "b", resp = cues),
#   # weakly regularizing prior for fixed effect coefficients for sigma.
#   set_prior("student_t(3, 0, 2.5)", class = "b", resp = cues, dpar = "sigma"),
#   set_prior("cauchy(0, 5)", class = "sd", resp = cues), 
#   set_prior("cauchy(0, 5)", class = "sd", resp = cues, dpar = "sigma"), 
#   set_prior("cauchy(0, 5)", class = "sd", resp = cues, group = randoms), 
#   set_prior("cauchy(0, 5)", class = "sd", resp = cues, group = randoms, dpar = "sigma"))
# 
# m.bivariate_distributional <- brm(
#   formula = bf_VOT + bf_f0_Mel + set_rescor(TRUE),
...
#   file = "../../models/bivariate_VOTf0_model.rds")  
```
...
plot_IO_fit <- function(
  ...
  #     select(-x) %>%
  #     unnest(categorization),
  #   mapping = aes(x = VOT, y = response, fill = gender),
  #   fun.data = mean_cl_boot,
  #   geom = "ribbon",
  #   alpha = .5) +
  scale_x_continuous("VOT (msec)", breaks = scales::pretty_breaks(n = 3), limits = c(-15, 80), expand = c(0, 0)) +
  ...
  geom_point(
    data = data %>%
      ...
      summarise(
        ...
        PSE.upper = round(quantile(PSE, probs = c(.975)))) %>%
      mutate(y = ifelse(gender == "male", -.025, - .06)),
    mapping = aes(x = PSE.median, y = y, color = gender), size = 1.2) +
      annotate(geom = "text",
           y = -.025, x = 65,
...
```{r, warning=FALSE}
io.talker.univar.no.noise <- get_IO_categorization(cues = c("VOT"), groups = c("Talker", "gender"), with_noise = FALSE, alpha = .1, size = 0.3) %>% 
  mutate(io.type = "io.talker.univar.no.noise")
io.gender.univar.no.noise <- get_IO_categorization(cues = c("VOT"), with_noise = FALSE, group = "gender", alpha = 1, size = 1) %>% 
  mutate(io.type = "io.gender.univar.no.noise")

PSE_table.univar.no.noise <- 
  io.talker.univar.no.noise %>%
  group_by(gender) %>%
  summarise(
    PSE.lower = round(quantile(PSE, probs = c(.025))),
    PSE.median = round(quantile(PSE, probs = c(.5))),
...
p.IOs.univar.no.noise <- plot_IO_fit(
  data = io.talker.univar.no.noise,
  data_group = io.gender.univar.no.noise,
  PSEs = PSE_table.univar.no.noise
)
```
...
io.talker.univar <- get_IO_categorization(cues = c("VOT"), groups = c("Talker", "gender"), alpha = .1, size = 0.3) %>% 
  mutate(io.type = "io.talker.univar")
io.gender.univar <- get_IO_categorization(cues = c("VOT"), group = "gender", alpha = 1, size = 1) %>% 
  mutate(io.type = "io.gender.univar")
...
p.IOs.univar <- plot_IO_fit(
  data = io.talker.univar,
  data_group = io.gender.univar,
  PSEs = PSE_table.univar
)  
```
...
io.talker.multivar <- get_IO_categorization(cues = c("VOT", "f0_Mel"), groups = c("Talker", "gender"), alpha = .1, size = .3) %>% 
  mutate(io.type = "io.talker.multivar")

io.gender.multivar <- get_IO_categorization(cues = c("VOT", "f0_Mel"), group = "gender", alpha = 1, size = 1) %>% 
  mutate(io.type = "io.gender.multivar")
...
p.IOs.multivar <- plot_IO_fit(
  data = io.talker.multivar,
  data_group = io.gender.multivar,
  PSEs = PSE_table.multivar
  )
```
...
```{r, warning=FALSE, fig.width=3.7, fig.height=2.5}
# using centered cues
io.talker.centered <- get_IO_categorization(
  cues = c("VOT_centered", "f0_Mel_centered"), 
  groups = c("Talker", "gender"), alpha = .1, size = .3) %>% 
  mutate(io.type = "io.talker.centered")
io.gender.centered <- get_IO_categorization(
  cues = c("VOT_centered", "f0_Mel_centered"),
  group = "gender", alpha = 1, size = 1) 

PSE_table.centered <- io.talker.centered %>%
  group_by(gender) %>%
  summarise(
    PSE.lower = round(quantile(PSE, probs = c(.025))),
    PSE.median = round(quantile(PSE, probs = c(.5))),
...
p.IOs.centered <- 
plot_IO_fit(
  data = io.talker.centered,
  data_group = io.gender.centered,
  PSEs = PSE_table.centered
)
```
...
legend <- get_legend(p.IOs.multivar + theme(legend.position = "top"))
...
```{r}
# join the 4 IO groups
d.talker.IO <- rbind(io.talker.univar.no.noise, io.talker.univar, io.talker.multivar, io.talker.centered) %>% 
  select(-c(PSE, line, categorization))

# get the likelihoods for each IO
temp <- d.talker.IO %>% 
  ungroup() %>%
  unnest(io) %>% 
  mutate(likelihood = 
           pmap(list(x, mu, Sigma, Sigma_noise), 
                ~ get_MVG_likelihood(x = ..1$x, mu = ..2, Sigma = ..3, noise_treatment = "marginalize", Sigma_noise = ..4)))   
...
# Make a LIST of multivariate Gaussian IAs from that IO 
# (with different kappas and nus)
m.ia.VOT_f0.PR <-
  crossing(
    prior_kappa = 4^(1:5),
    prior_nu = 4^(1:5)) %>%
  rowwise() %>%
  mutate(
    ideal_adaptor = map2(prior_kappa, prior_nu, ~ make_stop_VOTf0_ideal_adaptor(m = m.VOT_f0_MVG, kappa = .x, nu = .y))) %>%
