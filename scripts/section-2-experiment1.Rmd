...
# center the VOTs (the x-axis values) from the fitted model by adding the difference in database mean VOT and experiment's exposure VOT mean
...
p.IOs.VOT_F0.centered.input <- plot_IO_fit(
  ...
  PSE_VOT_F0.centered.input %>% 
    mutate(PSE.lower = round(PSE.lower - (chodroff.mean_VOT - VOT.mean_exp1)),
           PSE.median = PSE.median - (chodroff.mean_VOT - VOT.mean_exp1),
           PSE.upper = round(PSE.upper - (chodroff.mean_VOT - VOT.mean_exp1)))) 
...
ggsave("IOs_plot_exp1.png", last_plot(), width = 16, height = 35, units = "cm", path = "~/Desktop/")
```
...
d.test.IO <- 
  ...
  mutate(
    Item.VOT_F0 = map2(Item.VOT, Item.Mel_f0_5ms, ~ c(.x, .y)),
    ...
    Item.VOT_F0_centered = map2(Item.VOT_centered, Item.Mel_f0_centered, ~ c(.x, .y)),
...
if (file.exists("../data/d.test.IO.rds")) {
  d.test.IO <- read_rds("../data/d.test.IO.rds")
} else {
  d.test.IO %<>% mutate(
    log_likelihood_per_response =
      pmap(
        ...
        ~ get_average_log_likelihood_of_perception_data_under_IO(
          ...
          else if (str_detect(..3, "VOT_F0\\.centered\\.input$")) {..1$Item.VOT_F0_centered} 
          else if (str_detect(..3, "VOT_F0\\.no\\.noise$")) {..1$Item.VOT_F0} 
          else if (str_detect(..3, "VOT_F0\\.centered$")) {..1$Item.VOT_F0}
          else if (str_detect(..3, "VOT_F0$")) {..1$Item.VOT_F0}
          else if (str_detect(..3, "VOT\\.no\\.noise$")) {..1$Item.VOT}
          else if (str_detect(..3, "VOT\\.centered$")) {..1$Item.VOT}
          else if (str_detect(..3, "VOT$")) {..1$Item.VOT}, 
          observed_responses = ..1$Response.category, 
          model = ..2)) %>% 
      unlist(),
    likelihood_per_response = exp(log_likelihood_per_response)) %>% 
  mutate(
    io.type = fct_relevel(io.type, c("VOT.no.noise", "VOT", "VOT.centered", "VOT.centered.input", "VOT_F0.no.noise", "VOT_F0", "VOT_F0.centered", "VOT_F0.centered.input")),
    gender = fct_relevel(gender, c("female", "male")))
  
  write_rds(d.test.IO, "../data/d.test.IO.rds")
}
...
(ref:comparing-likelihoods-of-perception-data-under-each-IO) Comparing the likelihood of production IOs with different assumptions

```{r IO-univar-likelihood-plot, fig.width=4.5, fig.height=4.5, fig.cap="(ref:comparing-likelihoods-of-perception-data-under-each-IO)"}
...
IO.likelihood <- d.test.IO %>% 
  ...
  mutate(cues = ifelse(str_detect(io.type, "VOT_F0.*"), "VOT_F0", "VOT"))

p.likelihood <- IO.likelihood %>% 
  mutate(io.label = case_when(
    io.type == "VOT.no.noise" | io.type == "VOT_F0.no.noise" ~ "raw",
    io.type == "VOT" | io.type == "VOT_F0" ~ "+perceptual noise", 
    io.type == "VOT.centered" | io.type == "VOT_F0.centered" ~ "+talker-centered",
    io.type == "VOT.centered.input" | io.type == "VOT_F0.centered.input" ~ "+exposure-centered")) %>% 
  ...
  geom_linerange(
    data = d.bootstrap %>%
      ...
      mutate(
        cues = ifelse(str_detect(io.type, "VOT_F0.*"), "VOT_F0", "VOT"),
        io.label = case_when(
          io.type == "VOT.no.noise" | io.type == "VOT_F0.no.noise" ~ "raw",
          io.type == "VOT" | io.type == "VOT_F0" ~ "+perceptual noise", 
          io.type == "VOT.centered" | io.type == "VOT_F0.centered" ~ "+talker-centered",
